...
} = require('./setup-app.help');

function setupTestCommand(program) {
  program.command('test <appKey|systemKey>')
    .description('Tests: builder appKey in container; external systemKey = local validation')
...
async function runTestE2ECommand(appName, options, cmd) {
  const { cliOptsSkipSync } = require('../utils/cli-sync-options');
  const noSync = cliOptsSkipSync(options, cmd);
  const pathsUtil = require('../utils/paths');
  const appType = await pathsUtil.detectAppType(appName).catch(() => null);
  if (noSync && appType && appType.baseDir === 'builder') {
    throw new Error(
  ...
  if (appType && appType.baseDir === 'integration') {
    await runExternalIntegrationE2EAndCertSync(appName, options, cmd);
    return;
  }
...
function setupInstallCommand(program) {
  program.command('install <appKey>')
    .description('Install deps in container (builder appKey only)')
    ...
    .action(async(appName, options) => {
      try {
        ...
        if (appType && appType.baseDir === 'integration') {
          logger.log(
            chalk.gray('Install is for builder appKey only. Use aifabrix shell <appKey> for builder apps or External Systems commands for integrations.')
          );
          return;
        }
...
function setupTestE2eCommand(program) {
  program.command('test-e2e <appKey|systemKey>')
    .description('E2E: builder appKey in container; external systemKey = all datasources via dataplane')
    ...
    .option(
      '--force',
      'With default sync, pass force to upload (bypass active-datasource schema replacement guards; same as aifabrix upload --force)'
    )
    ...
    .action(async(appName, options, cmd) => {
      try {
        ...
        await runTestE2ECommand(appName, externalOpts, cmd);
      } catch (error) {
...
function setupLintCommand(program) {
  program.command('lint <appKey>')
    .description('Lint in container (builder appKey only)')
    .option('--env <env>', 'dev (running container) or tst (ephemeral with .env)', 'dev')
    .action(async(appName, options) => {
      try {
        const pathsUtil = require('../utils/paths');
        const appType = await pathsUtil.detectAppType(appName).catch(() => null);
        if (appType && appType.baseDir === 'integration') {
          logger.log(chalk.gray('lint is for builder appKey only. Use aifabrix shell <appKey> then make lint or pnpm lint.'));
          return;
        }
...
function setupTestGovernanceCommand(program) {
  const { testGovernanceCommandHandler } = require('../commands/test-governance-command-action');
  program
    .command('test-governance <appKey|systemKey>')
    .description(
      'Governance scenario acceptance: ABAC visibility via governed search (key sets only)'
    )
    .option('-e, --env <env>', 'Environment: dev, tst, or pro', 'dev')
    .option('--pack <path>', 'Scenario pack YAML (default: scenarios/default.yaml or <systemKey>-v1.yaml)')
    .option('-a, --app <path>', 'Integration folder override')
    .option(
      '--scenario <id>',
      'Run subset of scenario ids (repeatable)',
      collectScenarioId,
      []
    )
    ...
    .option('-v, --verbose', 'Per-scenario verdict lines with subject and counts')
    .option('--json', 'Machine-readable redacted result (same allowlist as API)')
    .addHelpText('after', TEST_GOVERNANCE_HELP_AFTER)
    .action((appName, options, cmd) => testGovernanceCommandHandler(appName, options, cmd));
}

function setupTestTrustCommand(program) {
  const { testTrustCommandHandler } = require('../commands/test-trust-command-action');
  program
    .command('test-trust <appKey|systemKey>')
    .description(
      'Semantic agent metadata validation for external system (all datasources via dataplane 404.5)'
    )
    .option('-e, --env <env>', 'Environment: dev, tst, or pro', 'dev')
    .option('-v, --verbose', 'Show findings and observed business model')
    ...
    .option(
      '--timeout <ms>',
      'HTTP timeout for validate/latest requests (ms)',
      '120000'
    )
    .option('--strict', 'Exit 1 unless every datasource is trusted')
    .option('--warnings-as-errors', 'Exit 1 when any datasource is usableWithWarnings')
    .option('--json', 'Print JSON array of trust runs')
    .option('--summary', 'One line per datasource')
    .addHelpText('after', TEST_TRUST_HELP_AFTER)
    .action((appName, options, cmd) => testTrustCommandHandler(appName, options, cmd));
}
...
function setupInstallTestE2eLintCommands(program) {
  ...
  setupTestGovernanceCommand(program);
  setupTestTrustCommand(program);
