...
Examples:
  # Local — builder app (builder/<appKey>/)
  $ aifabrix deploy myapp
  $ aifabrix deploy myapp --local

  # Local — external system (integration/<systemKey>/); manifest regenerated on deploy
  $ aifabrix deploy hubspot-demo
  $ aifabrix deploy hubspot-demo --local
  $ aifabrix deploy hubspot-demo --probe

  # CI/CD — builder app (GitHub Actions / Azure DevOps; pipeline client credentials in secrets)
  $ aifabrix validate myapp
  $ aifabrix build myapp
  $ aifabrix push myapp --registry $ACR_REGISTRY --tag $IMAGE_TAG
  $ aifabrix deploy myapp \\
      --client-id "$DEV_MISO_CLIENTID" \\
      --client-secret "$DEV_MISO_CLIENTSECRET" \\
      --no-poll

  # CI/CD — external integration (after upload/tests; promote to controller)
  $ aifabrix validate hubspot-demo
  $ aifabrix upload hubspot-demo --probe
  $ aifabrix deploy hubspot-demo --probe --no-poll

Notes:
  - Login or set MISO_CONTROLLER_URL + environment (aifabrix login / aifabrix auth).
  - CI: register once (aifabrix app register <appKey>), store DEV_MISO_CLIENTID and DEV_MISO_CLIENTSECRET in pipeline secrets.
  - upload = dataplane publish for dev iteration; deploy = controller validate + deploy (Azure or local Docker).
  - --repository-url overrides repo URL for pipeline validate (default: application.yaml repository.repositoryUrl).
`;
