...
const { handleSecretsGet } = require('../commands/secrets-get');
...
Subcommands:
  list, get, set, remove, remove-all   User secrets.local.yaml (add --shared for shared/remote)
...
const SECRET_GET_HELP_AFTER = `
Examples:
  $ aifabrix secret get hubspot-demo/token --shared --exists
  $ aifabrix secret get myapp/clientSecret
  $ aifabrix secret get hubspot/apiKey --shared --json

  --exists   Script mode: exit 0 when the key is present and non-empty; no output.
  Default      Local user secrets.local.yaml only (same target as secret set without --shared).
  --shared     aifabrix-secrets store only (team file or remote API; same as secret set --shared).
`;
...
function setupSecretGetCommand(secretCmd) {
  secretCmd
    .command('get <key>')
    .description('Get one secret (local user file by default; --shared for team store; --exists for scripts)')
    .addHelpText('after', SECRET_GET_HELP_AFTER)
    .option('--shared', 'Read from shared secrets (config aifabrix-secrets or remote API)')
    .option('--exists', 'Exit 0 when key is present and non-empty; print nothing (for scripts)')
    .option('--json', 'Output JSON { key, exists, value }')
    .action(async(key, options) => {
      try {
        await config.ensureSecretsEncryptionKey();
        await handleSecretsGet(key, options);
      } catch (error) {
        handleCommandError(error, 'secret get');
        process.exit(1);
      }
    });
}
...
function setupSecretsCommands(program) {
  ...
  setupSecretGetCommand(secretCmd);
