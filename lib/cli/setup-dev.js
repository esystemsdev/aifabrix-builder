...
Categories:
  Local config   show, set-id, set-env-config, set-home, set-format
  Onboarding     init (first machine), refresh (sync settings / renew cert)
...
Prints two copy-paste commands for the developer: standard dev init, and an optional variant with
--add-hosts when DNS does not resolve the Builder Server hostname. Use --hosts-ip to fill in the LAN IP in that variant.
`;
...
function setupDevPathAndFormatCommands(dev) {
  ...
  dev
    ...
    .description('Set or clear aifabrix-home override in config.yaml')
