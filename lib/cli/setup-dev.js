...
const { runDevInit } = require('../commands/dev-init');
const {
  handleDevList,
  handleDevAdd,
  handleDevUpdate,
...
/**
 * Display developer configuration (local ports and config; remote keys when set).
 * Always shows environment and controller; other keys only when set (per plan 65).
...
async function displayDevConfig(devId) {
  ...
  optionalConfigVars.forEach(v => logger.log(`  ${v.key}: ${v.value}`));
  logger.log('');
}
