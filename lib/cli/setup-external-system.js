...
const {
  ...
  DOWNLOAD_HELP_AFTER,
  DELETE_HELP_AFTER
} = require('./setup-external-system-help');
...
function setupDownloadCommand(program) {
  program.command('download <systemKey>')
    ...
    .addHelpText('after', DOWNLOAD_HELP_AFTER)
...
function setupUploadCommand(program) {
  program.command('upload <systemKey>')
    .description('Validate, publish to dataplane, and register with controller (draft application); or upload .protection batch')
    .addHelpText('after', UPLOAD_HELP_AFTER)
    .option('--dry-run', 'Validate and build payload only; no API calls')
    .option('--no-sync', 'For .protection: skip datasource sync after upload')
    .option('--json', 'For .protection: JSON output')
    ...
    .option(
      '--force',
      'Bypass schema guards on active datasources during publish (dangerous; shows a warning)'
    )
    .action(async(systemKey, options) => {
      try {
        const { isProtectionScope } = require('../protection/scope');
        if (isProtectionScope(systemKey)) {
          const { uploadProtectionBatchCommand } = require('../protection/upload-batch');
          const logger = require('../utils/logger');
          const code = await uploadProtectionBatchCommand(
            {
              env: options.env,
...
function setupDeleteCommand(program) {
  program.command('delete <systemKey>')
    ...
    .addHelpText('after', DELETE_HELP_AFTER)
    ...
    .action(async(systemKey, options) => {
      try {
        ...
        if (isProtectionScope(systemKey)) {
          ...
          logger.error(
            formatBlockingError(
              'delete .protection is not supported. Use: aifabrix protection delete <datasourceKey>'
            )
          );
          process.exit(1);
        }
...
async function runExternalSystemTestIntegration(appName, options) {
  ...
  const opts = {
    ...
    noSync: cliOptsSkipSync(options)
  };
...
function setupExternalSystemTestCommands(program) {
  ...
  program.command('test-integration <appKey|systemKey>')
    .description('Integration tests: builder appKey in container; external systemKey via dataplane')
    ...
    .option(
      '--no-sync',
      'Skip publishing local integration files; use system config already on the dataplane'
    )
    .option(
      '--sync',
      'Publish local system and datasource files before tests (default for external integration; same as upload)'
    )
    .option(
      '--force',
      'With --sync, pass force to upload (bypass active-datasource schema replacement guards; same as aifabrix upload --force)'
    )
