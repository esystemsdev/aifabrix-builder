...
async function runUpInfraCommand(options) {
  ...
  const bundle = passwordBundleFromCliOptions(options);
...
function setupUpInfraCommand(program) {
  program.command('up-infra')
    ...
    .option('--adminPassword <password>', 'Override admin password (dev: all roles; pro manual-single)')
    .option('--adminEmail <email>', 'Override {{adminEmail}} default (e.g. pgAdmin login email)')
    .option('--infraAdminPassword <password>', 'Pro profile: infra DB/pgAdmin/Redis Commander password')
    .option('--keycloakAdminPassword <password>', 'Pro profile: Keycloak install admin password')
    .option('--platformAdminPassword <password>', 'Pro profile: platform UI login password (user admin)')
    .option('--syncAdminKv', 'Also sync {{adminPassword}} catalog keys into secrets.local.yaml (legacy)')
