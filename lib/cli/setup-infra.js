...
function setupUpPlatformCommand(program) {
  program.command('up-platform')
    ...
    .option(
      '-f, --force',
      'Reset CLI auth (clear all device/client tokens, set environment to dev), clean builder/keycloak, builder/miso-controller, builder/dataplane, then re-fetch from templates'
    )
    .action(async(options) => {
      try {
        if (options.force) {
