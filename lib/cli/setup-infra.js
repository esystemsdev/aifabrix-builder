    ...
    .action(async(options) => {
      try {
        if (options.force) {
          ...
          if (!options.verbose) {
            printUpPlatformForceSummary(forceSummary, cleanedApps);
          }
        }

        if (!options.verbose) {
          await runGuidedUpPlatform(options, handleUpMiso, handleUpDataplane, handleLogin);
          return;
        }
...
function printUpPlatformForceSummary(forceSummary, cleanedApps) {
  const check = chalk.green('✔');
  logger.log('');
  logger.log(chalk.bold('Clean installation files'));
  logger.log(chalk.gray(` ${check} Cleared ${forceSummary.deviceCleared} device token(s) and ${forceSummary.clientCleared} client token(s)`));
  logger.log(chalk.gray(` ${check} Environment set to ${forceSummary.environment}`));
  logger.log(chalk.gray(` ${check} Default controller set to ${forceSummary.defaultControllerUrl} (run aifabrix login after platform is up)`));
