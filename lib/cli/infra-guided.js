 ...
 */
async function assertDevInfraForGuidedDataplane() {
...
async function runGuidedUpDataplane(options, handleUpDataplane) {
  ...
  logger.log('');
  await assertDevInfraForGuidedDataplane();
...
async function runGuidedDataplaneInstallPhase(options, handleUpDataplane, handleLoginFn, controllerUrl) {
  ...
  emitSystemBuilderAppManifestLineIfTTY(logger, 'dataplane');
  const dpSpin = startSpinner('Starting Dataplane...');
  const platformMode = await config.getSetupPlatformMode();
  if (platformMode === 'full') {
    // Full platform: three dataplane containers + per-env registrations.
    const envKeys = ['pro', 'dev', 'tst'];
    for (const envKey of envKeys) {
      await runGuidedDataplaneWithAuthRecovery(
        { ...options, envKey, runEnvKey: envKey, allowNonDev: true },
        handleUpDataplane,
        handleLoginFn,
        controllerUrl
      );
