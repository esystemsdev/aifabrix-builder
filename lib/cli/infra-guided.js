...
async function runGuidedAuthStep(handleLogin) {
  ...
  try {
    ...
    await handleLogin({
      ...
      environment: 'dev',
      compact: true,
      // Dataplane is not up yet during guided platform/dataplane install; refresh after it starts.
  ...
  // Compact device-login prints "Authentication successful"; close the step spinner cleanly.
  stopSpinnerSuccess(spin, 'Authenticated');
}
