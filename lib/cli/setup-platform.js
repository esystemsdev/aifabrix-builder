...
What this command does:
  - With no infra running: prompts for installation profile (dev = one password for infra + Keycloak + platform UI; pro = autogen or manual split passwords). Credentials are stored in admin-secrets.env (encrypted secure:// values), not in secrets.local.yaml unless you pass --syncAdminKv on up-infra. The admin email is saved in config.yaml for the next run. Prompts for an AI tool (OpenAI or Azure OpenAI) only when keys are not already set, then runs up-infra and up-platform.
...
function registerSetup(program) {
  program
    ...
    .option('--platform <mode>', 'Platform mode: single (direct ports) or full (Traefik front door + multi-env)')
    .option('--installation <profile>', 'Installation profile: dev (one password) or pro (autogen/manual)')
    .option('--pro-password-mode <mode>', 'Pro profile only: autogen or manual')
    .option('--admin-email <email>', 'Admin email (non-interactive fresh install)')
    .option('--admin-password <password>', 'Admin password for dev or pro manual-single (non-interactive)')
