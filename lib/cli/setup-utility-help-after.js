...
Examples:
  ...
  $ aifabrix repair hubspot-demo --auth oauth2
    Set authentication method; updates the system file and env.template (oauth2, aad, apikey, bearerToken, basic, …).
...
const RESOLVE_HELP_AFTER = `
Examples:
  $ aifabrix resolve hubspot-demo
    External integration: resolves integration/<systemKey>/env.template to .env using local host ports (no Docker rewrite).

  $ aifabrix resolve dataplane --fresh
    Regenerate the full .env from env.template (no merge with an existing file). Use after adding many new variables or when a normal resolve did not pick up new template lines.

  $ aifabrix secret set hubspot-demo/apiKey "<your-token>"
    Store a secret when resolve reports a missing kv:// reference.

  If a key is not required by your system file, comment out its line in env.template instead of creating a secret.
  $ aifabrix repair hubspot-demo
    Align env.template with authentication.security and comment stale OAuth KV lines after --auth bearerToken.
`;
