...
function setupRepairCommand(program) {
  program.command('repair <app>')
    .description('Repair external integration config: fix drift (file lists, app key, rbac, manifest)')
    ...
    .action(async(appName, options) => {
      try {
        ...
        logOfflinePathWhenType(appPath);
        const result = await repairExternalIntegration(appName, { dryRun: options.dryRun });
