...
/**
 * Candidate bases for relative openapi-file paths (first existing wins).
 * @param {string} configPath - Path to wizard.yaml
 * @returns {string[]}
 */
function wizardOpenApiFilePathCandidates(filePath, configPath) {
...
/**
 * Resolve openapi-file path for wizard config (absolute path).
 * Relative paths: process.cwd() first, then wizard.yaml directory.
 * @param {string} filePath
 * @param {string} configPath
 * @returns {Promise<string>}
 */
async function resolveWizardOpenApiFilePath(filePath, configPath) {
  const candidates = wizardOpenApiFilePathCandidates(filePath, configPath);
 ...
 * @param {string} configPath - Path to wizard.yaml (relative paths resolve from cwd, then config dir)
 * @returns {Promise<Object>} Validation result with valid flag, errors, and resolvedPath
 */
async function validateFilePath(filePath, configPath) {
  const resolvedPath = await resolveWizardOpenApiFilePath(filePath, configPath);
  if (!resolvedPath) {
    return { valid: false, errors: ['OpenAPI file path is required'], resolvedPath: '' };
  }
  try {
    await fs.access(resolvedPath);
  } catch (error) {
  ...
  return { valid: true, errors: [], resolvedPath };
}
...
async function performSemanticValidations(config, configPath, validateFilePaths) {
  ...
  if (validateFilePaths && config.source?.type === 'openapi-file' && config.source?.filePath) {
    const fileResult = await validateFilePath(config.source.filePath, configPath);
    if (!fileResult.valid) {
      errors.push(...fileResult.errors);
    } else if (fileResult.resolvedPath) {
      config.source.filePath = fileResult.resolvedPath;
    }
  }
