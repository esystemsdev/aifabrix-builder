...
function buildManifestStructure(appName, variables, deploymentKey, configuration, rbac) {
  ...
  // Add optional fields from variables if present
  if (variables.repository) {
    deployment.repository = variables.repository;
  }
  if (variables.build) {
    deployment.build = variables.build;
  }
  if (variables.deployment) {
    deployment.deployment = variables.deployment;
  }
