...
const { loadRequiresDatabasesArray, loadShippedRequiresDatabases } = require('./database-secret-values');

/** Platform apps from shipped templates (setup/up-infra before builder/ is copied). */
const SHIPPED_PLATFORM_APP_KEYS = ['keycloak', 'miso-controller', 'dataplane'];
...
/**
 * Database kv keys from shipped platform templates when builder/ apps are not copied yet
 * (e.g. `aifabrix setup` fresh install before `up-platform`).
 *
 * @returns {string[]}
 */
function deriveDatabaseKvKeysFromShippedPlatformTemplates() {
  const keys = new Set();
  for (const appKey of SHIPPED_PLATFORM_APP_KEYS) {
    const dbs = loadShippedRequiresDatabases(appKey);
    if (!Array.isArray(dbs) || dbs.length === 0) continue;
    for (let i = 0; i < dbs.length; i++) {
      keys.add(`databases-${appKey}-${i}-urlKeyVault`);
      keys.add(`databases-${appKey}-${i}-passwordKeyVault`);
...
function getAllInfraEnsureKeys(catalog, pathsUtil) {
  ...
  for (const k of deriveDatabaseKvKeysFromShippedPlatformTemplates()) set.add(k);
