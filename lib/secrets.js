...
async function generateMissingSecrets(envTemplate, secretsPath) {
  ...
  // Find all kv:// references in template
  const kvPattern = /kv:\/\/([a-zA-Z0-9-_]+)/g;
  const missingKeys = [];
  ...
  // Generate secure default values for missing keys
  const newSecrets = {};
  for (const key of missingKeys) {
    // Determine default value based on key name
    if (key.toLowerCase().includes('password')) {
      // Check if this is a database password (format: databases-{appName}-{index}-passwordKeyVault)
      const dbPasswordMatch = key.match(/^databases-([a-z0-9-_]+)-\d+-passwordKeyVault$/i);
      if (dbPasswordMatch) {
        const appName = dbPasswordMatch[1];
        // Convert appName to database name format (hyphens to underscores, like db-init does)
        const dbName = appName.replace(/-/g, '_');
        // Use the same password format as db-init container: {dbName}_pass123
        newSecrets[key] = `${dbName}_pass123`;
      } else {
        // Generate secure random password (32 bytes, base64 encoded = ~44 chars)
        newSecrets[key] = crypto.randomBytes(32).toString('base64');
      }
    } else if (key.toLowerCase().includes('url') || key.toLowerCase().includes('uri')) {
      // Check if this is a database URL (format: databases-{appName}-{index}-urlKeyVault)
      const dbUrlMatch = key.match(/^databases-([a-z0-9-_]+)-\d+-urlKeyVault$/i);
      if (dbUrlMatch) {
        const appName = dbUrlMatch[1];
        // Convert appName to database name format (hyphens to underscores, like db-init does)
        const dbName = appName.replace(/-/g, '_');
        // Generate DATABASE_URL template that uses ${DB_HOST} for environment-specific resolution
        // Format: postgresql://{dbName}_user:{dbName}_pass123@${DB_HOST}:5432/{dbName}
        newSecrets[key] = `postgresql://${dbName}_user:${dbName}_pass123@\${DB_HOST}:5432/${dbName}`;
      } else {
        // For other URLs, use empty string or template pattern
        newSecrets[key] = '';
      }
    } else if (key.toLowerCase().includes('key') || key.toLowerCase().includes('secret') || key.toLowerCase().includes('token')) {
      // Generate secure random key/secret/token
      newSecrets[key] = crypto.randomBytes(32).toString('base64');
    } else {
  ...
  // Write updated secrets file, preserving YAML structure
...
async function generateEnvFile(appName, secretsPath, environment = 'local', force = false) {
  ...
  // If force flag is set, generate missing secrets
  if (force) {
  ...
  if (fs.existsSync(variablesPath)) {
    ...
    // Only copy .env to output path if envOutputPath is explicitly set and not null
    if (variables?.build?.envOutputPath && variables.build.envOutputPath !== null) {
      // Resolve from project root, not from builderPath
      // envOutputPath is relative to project root (e.g., "apps/test-py-app/.env")
      let outputPath = path.resolve(process.cwd(), variables.build.envOutputPath);
      
      // If outputPath doesn't end with .env, treat it as a directory and append .env
      if (!outputPath.endsWith('.env')) {
        // Check if it's an existing directory
        if (fs.existsSync(outputPath) && fs.statSync(outputPath).isDirectory()) {
          outputPath = path.join(outputPath, '.env');
        } else {
          // If it doesn't exist, assume it's a directory path and append .env
          outputPath = path.join(outputPath, '.env');
        }
      }
