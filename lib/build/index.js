...
async function resolveDockerBuildArgsForApp(appName) {
  ...
  // Exported shell tokens win (local dev: export NPM_TOKEN=... before af build / docker build).
  const shellNpm = typeof process.env.NPM_TOKEN === 'string' ? process.env.NPM_TOKEN.trim() : '';
  const shellPypi = typeof process.env.PYPI_TOKEN === 'string' ? process.env.PYPI_TOKEN.trim() : '';
  if (shellNpm) buildArgs.NPM_TOKEN = shellNpm;
  if (shellPypi) buildArgs.PYPI_TOKEN = shellPypi;
  return buildArgs;
}
