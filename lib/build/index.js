...
async function buildApp(appName, options = {}) {
  ...
  try {
    ...
    // 5. Same kv:// backfill as `aifabrix run` (env.template → primary user secrets + merged shared)
    await ensureRunSecretsForApp(appName);

    // 6. Resolve NPM_TOKEN/PYPI_TOKEN for private registries and pass as build-args
    ...
    // 7. Execute Docker build (single tag per invocation)
