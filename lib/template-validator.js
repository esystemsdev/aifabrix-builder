...
async function validateTemplate(templateName) {
  ...
  if (!fsSync.existsSync(templatePath)) {
    throw new Error(`Template '${templateName}' not found. Expected folder: templates/${templateName}/`);
  }
...
async function listAvailableTemplates() {
  ...
  for (const entry of entries) {
    ...
    if (stats.isDirectory()) {
      // Skip certain directories that are not templates
      if (!['github', 'infra', 'python', 'typescript', 'template'].includes(entry)) {
