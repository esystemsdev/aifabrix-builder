...
async function updateTemplateVariables(appPath, appName, options, config) {
  ...
  try {
    ...
    // Clear envOutputPath from template - we don't copy .env files to external paths
    // .env file stays in builder/{appName}/ directory
    if (variables.build && variables.build.envOutputPath) {
      variables.build.envOutputPath = null;
    }

    // Note: context will be set correctly in createApp when --app flag is used

    // Replace template database names with app name when creating standalone app with --app flag
    // Template may have databases like "miso" and "miso-logs", but new app should use app name
    if (options.app && variables.requires && variables.requires.databases) {
      // Replace all database names with the app name
      variables.requires.databases = [{ name: appName }];
    } else if (options.app && variables.requires && variables.requires.database && !variables.requires.databases) {
      // If database is enabled but no databases array exists, create one with app name
      variables.requires.databases = [{ name: appName }];
    }
...
async function createApp(appName, options = {}) {
  try {
    ...
    // Configuration files always go in builder/ folder
    ...
    // If --app flag is used, also check if apps/ directory exists
    if (options.app) {
    ...
    // If --app flag is used, create apps/<appName>/ directory and copy application files there
    if (options.app) {
      // Create apps/<appName>/ directory
      const appsPath = path.join(process.cwd(), 'apps', appName);
      await fs.mkdir(appsPath, { recursive: true });

      // Update variables.yaml: set context to .. (relative from builder/ to root) and envOutputPath
      try {
        const yaml = require('js-yaml');
        const variablesPath = path.join(appPath, 'variables.yaml');
        const variablesContent = await fs.readFile(variablesPath, 'utf8');
        ...
        // Set context to ../.. (relative from builder/<appName>/ to project root)
        // Set envOutputPath to apps/<appName>/.env so Docker can include .env file
        if (variables.build) {
      ...
      // Copy application files to apps/<appName>/
      const language = config.language || options.language;
      if (!language) {
        // Try to get language from variables.yaml if it exists
        try {
          const yaml = require('js-yaml');
          const variablesPath = path.join(appPath, 'variables.yaml');
          const variablesContent = await fs.readFile(variablesPath, 'utf8');
          ...
          if (languageFromYaml) {
            const copiedFiles = await copyAppFiles(languageFromYaml, appsPath);
          ...
          throw new Error(`Failed to copy app files: ${error.message}`);
        }
      } else {
