...
function mergeTemplateVariables(options, templateVariables) {
  ...
  // Map template variables to options format
  // Port: use template port if not specified in options
  if (!merged.port && templateVariables.port) {
    merged.port = templateVariables.port;
  }

  // Language: use template build.language if not specified in options
  ...
  // Authentication: check if template has authentication section
...
async function createApp(appName, options = {}) {
  try {
    ...
    if (options.template) {
      ...
      const templatePath = path.join(__dirname, '..', 'templates', 'applications', options.template);
      ...
      try {
        ...
        templateVariables = yaml.load(templateContent);
      } catch (error) {
