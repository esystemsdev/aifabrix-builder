...
const { resolveHealthCheckPathWithFrontDoorVdir } = require('../utils/compose-generator');
...
function buildHealthCheck(variables) {
  ...
  const rawHealthPath = typeof hc.path === 'string' && hc.path.startsWith('/') ? hc.path : '/health';
  const path = resolveHealthCheckPathWithFrontDoorVdir(variables, 0, null, null);
  const healthCheck = {
    path,
  ...
  if (hc.probePath) {
    if (hc.probePath === rawHealthPath && path !== rawHealthPath) {
      healthCheck.probePath = path;
    } else {
      healthCheck.probePath = hc.probePath;
    }
  } else if (path !== rawHealthPath) {
    healthCheck.probePath = path;
  }
