 ...
 */
/**
 * Maps legacy integration auth method names to schema 1.3.0 values before upload/deploy.
 * @param {Object} systemJson - System JSON object (mutated in place)
 */
function normalizeLegacyAuthMethod(systemJson) {
  const auth = systemJson && systemJson.authentication;
  if (!auth || typeof auth !== 'object') return;
  const method = String(auth.method || '').trim().toLowerCase();
  if (method === 'bearerkey') {
    auth.method = 'bearerToken';
  }
}
...
async function loadSystemWithRbac(appPath, schemaBasePath, systemFile) {
  ...
  normalizeLegacyAuthMethod(systemJson);
