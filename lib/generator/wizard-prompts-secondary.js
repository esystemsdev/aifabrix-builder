...
function derivePreviewFromConfig(systemConfig, datasourceConfigs) {
  ...
  const endpointCount = sys.openapi?.endpoints?.length ?? sys.openapi?.operations
    ? Object.keys(sys.openapi.operations || {}).length
    : null;
  ...
  if (typeof attrs === 'object' && !Array.isArray(attrs)) {
    mappedFields = Object.keys(attrs);
    fieldMappingCount = mappedFields.length;
  }
  ...
  return {
    ...
    datasourceSummary: dsList.length > 0 ? {
      key: ds.key,
    ...
    fieldMappingsSummary: fieldMappingCount > 0 ? {
      mappingCount: fieldMappingCount,
      mappedFields,
      unmappedFields: []
    } : null
  };
...
function formatPreviewSummary(preview) {
  ...
  if (preview.datasourceSummary) parts.push(..._formatDatasource(preview.datasourceSummary));
...
async function promptForConfigReview({ preview, systemConfig, datasourceConfigs }) {
  const hasSummary = preview && (preview.systemSummary || preview.datasourceSummary);
