...
async function promptForExistingSystem(systemsList = [], defaultValue) {
  ...
  if (list.length > 0) {
    const choices = list.map((s) => {
      ...
      const name = (s.displayName ?? s.name ?? value) || value;
 ...
 * @param {Array<{key?: string, id?: string, credentialKey?: string, displayName?: string, name?: string}>} [credentialsList] - Credentials from GET /api/v1/wizard/credentials (or empty/null on error)
...
async function promptForExistingCredential(credentialsList = []) {
  ...
  if (list.length > 0) {
    const choices = list.map((c) => {
      ...
      const name = (c.displayName ?? c.name ?? value) || value;
      return { name: String(name), value };
    }).filter((c) => c.value);
    if (choices.length === 0) {
