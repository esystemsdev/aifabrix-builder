...
async function startInfra() {
  ...
  try {
    console.log(`Using compose file: ${composePath}`);
...
async function checkInfraHealth() {
  ...
  // Helper to find container by name pattern
  const findContainer = async(serviceName) => {
    try {
  ...
  for (const service of servicesWithHealthCheck) {
    try {
      const containerName = await findContainer(service);
      if (containerName) {
        ...
        health[service] = (status === 'healthy' || status === 'starting') ? 'healthy' : status;
      } else {
  ...
  for (const service of servicesWithoutHealthCheck) {
    try {
      ...
      if (containerName) {
        ...
        health[service] = (status === 'running' || status === 'healthy') ? 'healthy' : 'unhealthy';
      } else {
        health[service] = 'unknown';
      }
    } catch (error) {
