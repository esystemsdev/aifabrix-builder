...
const { readRelaxedCatalogDefaults } = require('../parameters/infra-parameter-catalog');
...
const { syncAdminPasswordKeysToStore } = require('./admin-secrets-sync');
const {
  DEFAULT_ADMIN_OBJ,
  writeAdminSecretsFile: writeAdminSecretsFileImpl
} = require('./admin-secrets-apply');
const { ensureAdminSecrets: ensureAdminSecretsImpl } = require('./admin-secrets-ensure');
...
async function applyAdminSecretsUpdate(
  ...
  merged.KEYCLOAK_ADMIN_USERNAME = merged.KEYCLOAK_ADMIN_USERNAME || 'admin';
  merged.KEYCLOAK_ADMIN_PASSWORD = passwordToUse;
  merged.PLATFORM_ADMIN_PASSWORD = passwordToUse;
  if (emailOpts && emailOpts.updateEmail && emailOpts.emailToUse) {
    merged.PGADMIN_DEFAULT_EMAIL = emailOpts.emailToUse;
  }
  await writeAdminSecretsFileImpl(getCoreSecrets, adminSecretsPath, merged, null);
  if (shouldOverwriteWithAdminPwd) {
    logger.log('Updated admin password in admin-secrets.env.');
    if (syncAdminKv) {
      await syncAdminPasswordKeysToStore(passwordToUse);
    }
...
/** @param {Object} [options] @returns {Promise<string>} */
async function ensureAdminSecrets(options = {}) {
  return ensureAdminSecretsImpl(
    getCoreSecrets,
    getInfraDirName,
    logVolumeResetHint,
