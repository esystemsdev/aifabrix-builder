...
async function repairExternalIntegration(appName, options = {}) {
  ...
  const { variables, originalYamlContent, systemFiles, datasourceFiles } = loadConfigAndDiscover(appPath, configPath);
  const changes = [];
  ...
  const rbacMergeUpdated = options.rbac
    ...
    : false;
  const anyUpdated = steps.updated || datasourceRepairUpdated || rbacMergeUpdated;
