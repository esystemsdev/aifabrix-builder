...
const { ensureSystemPermissionsForAutoRbac } = require('./repair-system-permissions');
...
function runRepairSteps(ctx) {
  ...
  const systemPermissionsEnsured = ensureSystemPermissionsForAutoRbac(
    ctx.appPath,
    ctx.systemParsed,
    ctx.datasourceFiles,
    ctx.dryRun,
  ...
  const systemFileUpdated =
    authReplaced || systemAuthConfigNormalized || systemDataSourcesAligned || authVarsRemoved || systemPermissionsEnsured;
