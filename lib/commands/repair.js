...
async function repairExternalIntegration(appName, options = {}) {
  ...
  const { updated: keysNormalized, datasourceFiles } = normalizeDatasourceKeysAndFilenames(appPath, initialDatasourceFiles, systemKey,
  ...
  const rbacMigratedFromSystem = options.rbac
    ? migrateSystemRbacIntoRbacFile(appPath, systemFilePath, systemParsed, { dryRun, changes, backupCtx })
    : false;
  const rbacMergeUpdated = options.rbac
    ...
    : false;
  const anyUpdated = keysNormalized || steps.updated || datasourceRepairUpdated || rbacMigratedFromSystem || rbacMergeUpdated;
  const manifestRegenerated = (anyUpdated && !dryRun)
    ? await persistChangesAndRegenerate({ configPath, variables, appName, appPath, changes, originalYamlContent, backupCtx })
    : false;
  const readmeRegenerated = await regenerateReadmeIfRequested(appName, appPath, { ...options, backupCtx }, changes);
  return buildRepairResult(steps, anyUpdated, manifestRegenerated, readmeRegenerated,
    { changes, systemFiles, datasourceFiles, backupPaths });
}
