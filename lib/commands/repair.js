...
async function repairExternalIntegration(appName, options = {}) {
  ...
  /** With --rbac, relocate roles/permissions from *-system.json into rbac.json|yaml before datasource merge. */
  const rbacMigratedFromSystem = options.rbac
    ? migrateSystemRbacIntoRbacFile(appPath, systemFilePath, systemParsed, { dryRun, changes })
    : false;
