...
function makeUniqueBackupPath(builderRoot, appName, suffix) {
  const base = path.join(builderRoot, `${appName}.backup-${suffix}`);
  if (!fs.existsSync(base)) return base;
  for (let i = 2; i < 1000; i++) {
    const candidate = path.join(builderRoot, `${appName}.backup-${suffix}-${i}`);
    if (!fs.existsSync(candidate)) return candidate;
  }
  throw new Error(`Could not create unique backup path for ${appName} under ${builderRoot}`);
}

function backupPlatformAppDirs(builderRoot, platformApps) {
  const suffix = formatBackupSuffix();
  const backedUp = [];
  for (const appName of platformApps) {
    const appDir = path.join(builderRoot, appName);
    if (!isExistingNonEmptyDir(appDir)) continue;
    const dest = makeUniqueBackupPath(builderRoot, appName, suffix);
...
function handleBuilderDirAction(action, builderRoot) {
  ...
  if (action === setupPrompts.BUILDER_DIR_ACTION.BACKUP) {
    const backedUp = backupPlatformAppDirs(builderRoot, PLATFORM_APPS);
    for (const item of backedUp) {
      logger.log(
        chalk.gray(
          ` ${successGlyph()} Backed up builder/${item.appName}/ → ${path.basename(item.to)}/`
        )
      );
    }
...
async function maybePromptForExistingBuilderDir({ force }) {
  ...
  if (!existing) return { aborted: false, backedUp: [], skipClean: false };
  const { builderRoot, entries } = existing;
  const action = await setupPrompts.promptBuilderDirConflict({
    builderRoot,
    totalEntries: entries.length,
    platformApps: PLATFORM_APPS
  });
  return handleBuilderDirAction(action, builderRoot);
}
