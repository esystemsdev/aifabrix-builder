...
const { infoLine, successGlyph } = require('../utils/cli-test-layout-chalk');
...
const { ensureSetupPlatformAuth } = require('./setup-platform-auth');
...
async function runUpPlatform(opts = {}) {
  ...
  let authCtx;
  try {
    authCtx = await ensureSetupPlatformAuth({ applyForceConfig: opts.force === true });
  } catch (authErr) {
...
async function runFreshInstall(adminCreds) {
  ...
  const resetSpin = startSpinner('Removing previous infrastructure data...');
  await withMutedLogger(async() => {
...
async function runReinstall() {
  ...
  stopSpinnerSuccess(infraSpin, 'Infrastructure ready');
  await runUpPlatform({ force: true, skipBuilderConflictPrompt: true });
}
