...
const DIMENSION_VALUE_HELP_AFTER = `
...
Per-command help:  aifabrix dimension value <command> --help
`;
...
/**
 * @param {Object} response - Controller API response
 * @throws {Error} When response.success === false
 */
function throwIfApiFailed(response) {
  if (!response || response.success !== false) {
    return;
  }
  throw new Error(response.formattedError || response.error || 'Request failed');
}
  ...
  cmd
    ...
    .action(async(dimensionValueId) => {
      try {
        ...
        const res = await deleteDimensionValue(controllerUrl, authConfig, id);
        throwIfApiFailed(res);
...
/**
 * @param {import('commander').Command} dimensionCmd - Parent `dimension` command
 */
function setupDimensionValueCommands(dimensionCmd) {
  const cmd = dimensionCmd
    .command('value')
    .description('Manage dimension values (static dimensions)')
    .addHelpText('after', DIMENSION_VALUE_HELP_AFTER);
