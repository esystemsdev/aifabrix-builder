 ...
 * - aifabrix dimension value create|list|delete
 *
...
const { readDimensionCreateFile, normalizeValueType } = require('../resolvers/dimension-file');
const {
  formatBlockingError,
  formatSuccessLine,
  infoLine,
...
const { setupDimensionValueCommands } = require('./dimension-value');

const DIMENSION_KEY_PATTERN = /^[a-zA-Z][a-zA-Z0-9-_]*$/;

/** Parent help for `aifabrix dimension -h` (leaf commands have their own Examples blocks). */
const DIMENSION_HELP_AFTER = `
...
Per-command help:  aifabrix dimension <command> --help
`;
...
function buildCreatePayload(options) {
  ...
  if (options.valueType) {
    payload.valueType = normalizeValueType(options.valueType);
  } else if (payload.valueType === undefined) {
    payload.valueType = 'static';
  }
...
function setupDimensionCreateCommand(dim) {
  dim
    ...
    .option('--value-type <mode>', 'static | dynamic | both (catalog assignment mode)')
    ...
    .action(async(options) => {
      try {
        ...
        const vt = dimRow.valueType || payload.valueType || 'static';
        if (out.created) {
          logger.log(formatSuccessLine(`Dimension created: ${dimRow.key} (valueType: ${vt})`));
          return;
        }
        logger.log(formatSuccessLine(`Dimension exists: ${dimRow.key} (valueType: ${vt})`));
  ...
  dim
    ...
    .action(async(dimensionIdOrKey, options) => {
      try {
        ...
        logger.log(headerKeyValue('Value type:', row.valueType || '—'));
...
function setupDimensionListCommand(dim) {
  function displayDimensionList(items, environment, controllerUrl) {
    ...
    const VTYPE_WIDTH = 8;
    const REQ_WIDTH = 10;
    const SEP_LEN = KEY_WIDTH + DISPLAY_WIDTH + TYPE_WIDTH + VTYPE_WIDTH + REQ_WIDTH;

    const keyCol = 'Key'.padEnd(KEY_WIDTH);
    const displayCol = 'Display'.padEnd(DISPLAY_WIDTH);
    const typeCol = 'Type'.padEnd(TYPE_WIDTH);
    const vtypeCol = 'VType'.padEnd(VTYPE_WIDTH);
    const reqCol = 'Required'.padEnd(REQ_WIDTH);
    logger.log(chalk.gray(`${keyCol}${displayCol}${typeCol}${vtypeCol}${reqCol}`));
    ...
    items.forEach((d) => {
      ...
      const valueType = (d?.valueType ?? '—').toString().padEnd(VTYPE_WIDTH);
      const required = (d?.isRequired ?? '—').toString().padEnd(REQ_WIDTH);
      logger.log(`${key}${displayName}${dataType}${valueType}${required}`);
    });
    logger.log('');
  }
...
function setupDimensionCommands(program) {
  const dim = program
    .command('dimension')
    .description('Manage the Controller Dimension Catalog')
    .addHelpText('after', DIMENSION_HELP_AFTER);
  setupDimensionCreateCommand(dim);
  setupDimensionGetCommand(dim);
  setupDimensionListCommand(dim);
  setupDimensionValueCommands(dim);
}
