...
/** Max wall time for post-login version cache refresh (silent after success banner). */
const LOGIN_POST_REFRESH_BUDGET_MS = 12000;

/** Per health probe during login refresh (two endpoints may be tried). */
const LOGIN_HEALTH_PROBE_TIMEOUT_MS = 6000;

/**
 * Resolve when `promise` settles or when `timeoutMs` elapses (returns undefined).
 * @template T
 ...
 * }>}
 */
async function refreshDataplaneVersionInfo(controllerUrl, dataplaneUrl, options = {}) {
  const cliVersion = getInstalledCliVersion();
  let dataplaneVersion;
  let minBuilderCliVersion;
  const healthOpts = {};
  if (options.healthTimeoutMs !== undefined && options.healthTimeoutMs !== null) {
    healthOpts.timeoutMs = options.healthTimeoutMs;
  }

  try {
    const snapshot = await fetchDataplaneGeneralHealth(dataplaneUrl, healthOpts);
    if (snapshot) {
 ...
 */
async function tryRefreshDataplaneVersionAfterLoginWork(controllerUrl, loginOptions = {}) {
  ...
  if (dataplaneUrl) {
    await refreshDataplaneVersionInfo(controllerUrl, dataplaneUrl, {
      healthTimeoutMs: LOGIN_HEALTH_PROBE_TIMEOUT_MS
    });
  }
}
...
async function tryRefreshDataplaneVersionAfterLogin(controllerUrl, loginOptions = {}) {
  ...
  try {
    await raceWithTimeout(
      tryRefreshDataplaneVersionAfterLoginWork(controllerUrl, loginOptions),
      LOGIN_POST_REFRESH_BUDGET_MS
    );
  } catch (_error) {
    // best-effort only
  }
