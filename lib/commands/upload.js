...
async function runUploadValidatePublish(dataplaneUrl, authConfig, payload) {
  ...
  const validateFailed = validateRes?.success === false || validateRes?.data?.success === false;
  if (validateFailed) {
    const errRes = validateRes?.success === false
      ? validateRes
      : { success: false, ...validateRes?.data, formattedError: validateRes?.data?.formattedError };
    const msg = formatApiError(errRes, dataplaneUrl);
    throw new Error(`Upload validation failed: ${msg}`);
  }

  const publishRes = await publishUploadViaPipeline(dataplaneUrl, uploadId, authConfig);
  const publishFailed = publishRes?.success === false || publishRes?.data?.success === false;
  if (publishFailed) {
    const errRes = publishRes?.success === false
      ? publishRes
      : { success: false, ...publishRes?.data, formattedError: publishRes?.data?.formattedError };
    const msg = formatApiError(errRes, dataplaneUrl);
    throw new Error(`Upload publish failed: ${msg}`);
  }
