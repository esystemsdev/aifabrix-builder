...
async function buildValidatedUploadManifestPayload(systemKey, opts = {}) {
  ...
  await syncDeployJsonFromSources(systemKey);
...
async function runUploadPublishAndSummary(systemKey, options, manifest, payload) {
  ...
  const controllerUrl = await resolveControllerUrl();
  // Plan 142.0: gate dataplane API entrypoints by the dataplane-advertised minimum Builder CLI version.
  await assertDataplaneCliVersionCompatible(dataplaneUrl, { controllerUrl });
  logger.log('');
  ...
  const rawRes = await runWithUploadPipelineSpinner(
    async() => {
      const res = await runUploadValidatePublish(dataplaneUrl, authConfig, payload, systemKey);
      await logAndSyncLocalOpenApiForUpload(systemKey, manifest);
      return res;
    },
    { systemKey, json: options.json === true }
  );
