...
function logForcePublishWarning() {
  logger.log(
    chalk.yellow(
      'Warning: --force bypasses Plan 331 schema replacement guards on active datasources. '
...
async function runUploadPublishAndSummary(systemKey, options, manifest, payload) {
  ...
  const rawRes = await runWithUploadPipelineSpinner(
    async() => {
      const res = await runUploadValidatePublish(dataplaneUrl, authConfig, payload);
