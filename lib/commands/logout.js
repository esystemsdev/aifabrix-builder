...
async function handleLogout(options) {
  ...
  let deviceTokensCleared = 0;
  let clientTokensCleared = 0;
  ...
  } else if (!options.environment && !options.app) {
    // Clear all device tokens (only when no environment/app specified)
    deviceTokensCleared = await clearAllDeviceTokens();
    if (deviceTokensCleared > 0) {
      logger.log(chalk.green(`✓ Cleared ${deviceTokensCleared} device token(s)`));
    } else {
  ...
  if (options.app && options.environment) {
    ...
    clientTokensCleared = await clearClientTokensForEnvironment(options.environment);
    if (clientTokensCleared > 0) {
      logger.log(chalk.green(`✓ Cleared ${clientTokensCleared} client token(s) for environment '${options.environment}'`));
    } else {
      logger.log(chalk.gray(`  No client tokens found for environment '${options.environment}'`));
    }
  } else if (!options.controller && !options.environment && !options.app) {
    // Clear all client tokens (only when no specific options specified)
    clientTokensCleared = await clearAllClientTokens();
    if (clientTokensCleared > 0) {
      logger.log(chalk.green(`✓ Cleared ${clientTokensCleared} client token(s)`));
    } else {
