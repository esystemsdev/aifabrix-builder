...
/**
 * Remove materialized `builder/` trees (aifabrix-work, aifabrix-home / config dir, cwd).
 * Complements {@link cleanAifabrixSystemDir} when `builder/` lives under `aifabrix-work` outside
 * the config directory.
 *
 * @returns {{ removed: string[], failed: string[] }}
 */
function cleanMaterializedBuilderTrees() {
  const candidates = [
    pathsUtil.getBuilderRoot(),
    path.join(pathsUtil.getAifabrixSystemDir(), 'builder'),
...
async function handleTeardown(options = {}) {
  ...
  try {
    ...
    const builderClean = cleanMaterializedBuilderTrees();
    for (const p of builderClean.removed) {
      removed.push(p);
    }
    for (const p of builderClean.failed) {
      failed.push(p);
    }
