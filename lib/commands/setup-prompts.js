...
/**
 * Prompt for admin credentials (dev or pro installation profile).
 *
 * @async
 * @param {Object} [options] - CLI options (`installation`, `proPasswordMode`, `adminEmail`, `adminPassword`, `quiet`)
 * @returns {Promise<{ adminEmail: string, adminPassword: string, passwordBundle: object, profile: string }>}
 */
async function promptAdminCredentials(options = {}) {
  const result = await promptAdminCredentialsWithProfile(options);
  const legacyPassword =
    result.passwordBundle.mode === 'single'
      ? result.passwordBundle.password
      : result.passwordBundle.platform;
  return {
    adminEmail: result.adminEmail,
    adminPassword: legacyPassword,
    passwordBundle: result.passwordBundle,
