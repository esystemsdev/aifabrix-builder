 ...
 * Subcommands `test`, `test-integration`, `test-e2e`, and `test-trust` call the dataplane; `log-*` read saved debug JSON locally; `clean-logs` removes those files. Permissions are summarized in `docs/commands/permissions.md`.
 *
...
const { setupDatasourceTestTrustCommand } = require('./datasource-test-trust-cli');
...
Examples:
  ...
  $ af ds test-trust <datasourceKey> --app <integrationFolder> -v
...
function setupDatasourceLogTrustCommand(datasource) {
  datasource.command('log-trust <datasourceKey>')
    .description('Show agent trust log from datasource test-trust (latest test-trust-*.json or --file)')
    .option(
      '-a, --app <app>',
      'Integration folder name (optional: resolve from cwd or datasource key if single match)'
    )
    .option('-f, --file <path>', 'Path to log file (default: latest test-trust log in app logs folder)')
    .action(async(datasourceKey, options) => {
      try {
        await runLogViewer(datasourceKey, {
          app: options.app,
...
function setupDatasourceCommands(program) {
  ...
  setupDatasourceTestTrustCommand(datasource);
