 ...
 * Subcommands `test`, `test-integration`, `test-e2e`, and `verify-trust` call the dataplane; `log-*` read saved debug JSON locally; `clean-logs` removes those files. Permissions are summarized in `docs/commands/permissions.md`.
 *
...
const { runCleanLogs } = require('../datasource/log-cleaner');
const {
...
const { setupDatasourceVerifyTrustCommand } = require('./datasource-test-trust-cli');
const { setupDatasourceVerifyAuditCommand } = require('./datasource-verify-audit-cli');
const { setupDatasourceLoadExportCommands } = require('./datasource-load-export-cli');
...
Examples:
  ...
  $ af ds test-e2e <datasourceKey> --verify-audit
  $ af ds verify-audit <datasourceKey>
  $ af ds verify-trust <datasourceKey> --app <integrationFolder> -v
  $ af ds load <datasourceKey> --app <integrationFolder> --dry-run
  $ af ds export <datasourceKey> --app <integrationFolder> --format ndjson -v
  ...
  $ aifabrix datasource clean-logs --app test-e2e-hubspot --dry-run
  $ aifabrix datasource clean-logs --all --type e2e
...
function setupDatasourceCleanLogsCommand(datasource) {
  datasource
    .command('clean-logs')
    .description('Remove saved test debug JSON under integration/<app>/logs/')
    .option('-a, --app <app>', 'Integration folder name (system key)')
    .option('--all', 'Clean logs under every integration/<app>/logs/ folder')
    .option(
      '-t, --type <type>',
      'Log kind: test, integration, e2e, trust, or all (default: all)',
      'all'
    )
    ...
    .action(async options => {
      try {
        ...
        logger.error(formatBlockingError('clean-logs failed:'), error.message);
        process.exit(1);
      }
    });
}

function setupDatasourceLogTrustCommand(datasource) {
  datasource.command('log-trust <datasourceKey>')
    .description('Show agent trust log from datasource verify-trust (latest verify-trust-*.json or --file)')
    .option(
      '-a, --app <app>',
      'Integration folder name (optional: resolve from cwd or datasource key if single match)'
    )
    .option('-f, --file <path>', 'Path to log file (default: latest verify-trust log in app logs folder)')
    .action(async(datasourceKey, options) => {
      try {
        await runLogViewer(datasourceKey, {
          app: options.app,
        ...
        logger.error(formatBlockingError('log-trust failed:'), error.message);
        process.exit(1);
      }
    });
}
...
function setupDatasourceCommands(program) {
  ...
  setupDatasourceLoadExportCommands(datasource);
  ...
  setupDatasourceVerifyAuditCommand(datasource);
  setupDatasourceVerifyTrustCommand(datasource);
  ...
  setupDatasourceLogTrustCommand(datasource);
  setupDatasourceCleanLogsCommand(datasource);
}
