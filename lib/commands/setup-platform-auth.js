 ...
 *
 * @async
 * @param {{ applyForceConfig?: boolean }} [opts]
 * @returns {Promise<{
...
async function ensureSetupPlatformAuth(opts = {}) {
  const storedPlatformUrl = await config.getPlatformControllerUrl();
  const platformControllerUrl = storedPlatformUrl || (await resolvePlatformControllerUrl());
  ...
  if (opts.applyForceConfig === true) {
    forceSummary = await upCommon.applyUpPlatformForceConfig({
      silent: true,
      clearTokens: !skipLoginIfAuthenticated,
      defaultControllerUrl: platformControllerUrl
    });
    if (forceSummary && forceSummary.deviceCleared > 0) {
