...
function buildAuthenticationForRepair(systemKey, authOption) {
  ...
  const schemaMethod =
    normalized === 'queryparam' ? 'queryParam' : normalized === 'bearerkey' ? 'bearerKey' : normalized;
...
function repairAuthChangeLabel(authOption) {
  const normalized = normalizeRepairAuthOption(authOption);
  if (normalized === 'bearerkey') return 'bearerKey';
  return normalized;
}

const MISSING_TEST_ENDPOINT_WARNING =
  'authentication.variables.testEndpoint is missing for apikey/bearerKey auth — add a GET URL for credential/E2E tests (e.g. /crm/v3/objects/contacts?limit=1, or a path resolved against baseUrl).';
...
function appendTestEndpointWarningIfMissing(authentication, warnings) {
  ...
  if (method !== 'apikey' && method !== 'bearerkey') return;
