...
async function handleConfigurationGeneration(dataplaneUrl, authConfig, options) {
  ...
  try {
    ...
    if (options.sourceType === 'known-platform' && options.platformKey) {
      ...
      generateResponse = await getPlatformConfig(dataplaneUrl, authConfig, options.platformKey, platformPayload);
    } else {
      ...
      generateResponse = await generateConfig(dataplaneUrl, authConfig, configPayload);
    }
    spinner.stop();
