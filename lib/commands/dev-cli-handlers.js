...
function logDevPinShareInstructions(p) {
  ...
  const pinOnly = `aifabrix dev activate --pin ${pin}`;
  const pinOnlyAlt = `aifabrix dev init --pin ${pin}`;
  logger.log(chalk.green(`✓ PIN created for ${id}, expires ${expiresAt}.`));
  logger.log('');
  logger.log(
  ...
  logger.log(`   ${pinOnly}`);
  logger.log(chalk.gray(`   (equivalent: ${pinOnlyAlt})`));
  logger.log('');
}
