...
Examples:
  ...
  $ aifabrix datasource test-e2e hubspot-users --sync --force --verify-audit -v
...
Notes:
  ...
  - To prove governance audit evidence after a green E2E, add --verify-audit (see verify-audit command).
`;
}
...
function attachDatasourceTestE2eExclusiveOptions(cmd) {
  return cmd
    ...
    .option(
      ...
      'Exit 1 when positional [capabilityKey] is set but the report lists more than one capabilities[] row'
    )
    .option(
      '--verify-audit',
      'After successful E2E, verify the nine-row audit evidence matrix on the dataplane (requires audit:read)'
    )
    .option(
      '--no-verify-audit',
      'Skip audit evidence verification even if enabled by default in a future release'
    )
    .option(
      '--audit-poll-ms <ms>',
      'Max wait for audit rows after E2E when using --verify-audit (default 15000)',
      '15000'
    )
    .option('--audit-poll-interval-ms <ms>', 'Audit poll interval when using --verify-audit (default 2000)', '2000');
}
