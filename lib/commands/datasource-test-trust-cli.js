...
async function writeTrustDebugLog(appKey, datasourceKey, payload, options) {
  if (!options || !options.debug) return;
  const logPath = await writeTestLog(
    appKey,
    payload,
    `test-trust-${datasourceKey}`,
...
async function datasourceTestTrustAction(datasourceKey, options) {
  try {
    const result = await runDatasourceTestTrustOnce(datasourceKey, options);
    ...
    if (options.debug) {
      await writeTrustDebugLog(appKey, datasourceKey, {
    ...
    const exitCode = await finalizeDatasourceTestTrust(datasourceKey, options, result);
...
function setupDatasourceTestTrustCommand(datasource) {
  const cmd = datasource
    ...
    .description('Semantic agent metadata validation for one datasource (dataplane 404.5)');
