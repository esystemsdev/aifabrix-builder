...
const { tryRefreshDataplaneVersionAfterLogin } = require('./auth-status-dataplane-version');
...
async function normalizeControllerUrl(options) {
  ...
  controllerUrl = String(controllerUrl).trim().replace(/\/+$/, '');
 ...
 * @param {boolean} skipRefresh
 * @returns {Promise<void>}
 */
async function refreshDataplaneVersionIfNeeded(controllerUrl, options, skipRefresh) {
  if (skipRefresh) return;
  if (!options.compact) {
    const { LOGIN_POST_REFRESH_BUDGET_MS } = require('./auth-status-dataplane-version');
    logger.log(
      chalk.gray(
        `Syncing dataplane version cache (optional, up to ${Math.round(LOGIN_POST_REFRESH_BUDGET_MS / 1000)}s)...`
      )
    );
  }
  await tryRefreshDataplaneVersionAfterLogin(controllerUrl, options);
}

/**
 * @returns {Promise<'credentials'|'device'>}
 */
async function executeLoginMethod(method, controllerUrl, environment, options, skipRefresh) {
  if (method === 'credentials') {
    await handleCredentialsLoginFlow(controllerUrl, environment, options);
    await refreshDataplaneVersionIfNeeded(controllerUrl, options, skipRefresh);
    return 'credentials';
  }
 ...
 */
function logLoginSuccessFooter(options, controllerUrl, environment) {
...
async function handleLogin(options) {
  ...
  const skipRefresh =
    options.skipDataplaneVersionRefresh === true || options.skipVersionCache === true;
  const flow = await executeLoginMethod(method, controllerUrl, environment, options, skipRefresh);
  if (flow === 'device') return;

  logLoginSuccessFooter(options, controllerUrl, environment);
}
