...
const { generateCSR, getCertDir } = require('../utils/dev-cert-helper');
...
/**
 * Save certificate and key to cert dir; set developer-id in config.
 ...
 */
async function saveCertAndConfig(configDir, devId, certificatePem, keyPem) {
...
async function runDevInit(options) {
  ...
  const configDir = getConfigDirForPaths();
  await saveCertAndConfig(configDir, devId, issueResponse.certificate, keyPem);
