...
async function installDevCaAndRetryHealth(baseUrl, options) {
  ...
  await installCaPlatform(caBuf, baseUrl);
...
async function runDevInit(options) {
  ...
  if (options.addHosts || options['add-hosts']) {
  ...
  let serverCaPemForTls;
  try {
    serverCaPemForTls = await ensureServerTrusted(baseUrl, options);
  } catch (err) {
  ...
  const configDir = getConfigDirForPaths();
  const caPem = issueResponse.caCertificate || issueResponse.ca || serverCaPemForTls;
...
async function runCertificateRefresh(auth) {
  ...
  const caPem = issueResponse.caCertificate || issueResponse.ca || serverCaPem;
  await saveCertAndConfig(configDir, devId, issueResponse.certificate, keyPem, caPem);
