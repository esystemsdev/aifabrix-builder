...
const { commentOrphanKvEnvLines } = require('../utils/env-kv-orphan-lines');
 ...
 * @returns {boolean}
 */
function createEnvTemplateIfMissing(envPath, expectedByKey, dryRun, changes, systemParsed, backupCtx) {
  ...
  trackRepairWrite(envPath, backupCtx);
  if (!dryRun) {
...
/**
 * Comments active KV_* lines whose env key is not in the system-derived expected set.
 * @param {string} content
 * @param {Map<string, string>} expectedByKey
 * @param {{ value: boolean }} changedRef
 * @returns {string}
 */
function commentOrphanKvLines(content, expectedByKey, changedRef) {
  const { content: next, changed } = commentOrphanKvEnvLines(content, expectedByKey);
  if (changed) {
    changedRef.value = true;
  }
  return next;
 ...
 */
function persistEnvTemplate(envPath, output, dryRun, backupCtx) {
  trackRepairWrite(envPath, backupCtx);
  if (!dryRun) {
 ...
 * @param {string} content
 * @param {Map<string, string>} expectedByKey
 * @returns {{ output: string, mergeChanged: boolean, pruneChanged: boolean }}
 */
function mergeAndPruneEnvTemplate(content, expectedByKey) {
  const mergeResult = mergeEnvTemplateContent(content, expectedByKey);
  const pruneChanged = { value: false };
  const output = commentOrphanKvLines(mergeResult.output, expectedByKey, pruneChanged);
  return {
    output,
    mergeChanged: mergeResult.changed,
    pruneChanged: pruneChanged.value
  };
}
...
function repairEnvTemplate(appPath, systemParsed, systemKey, dryRun, changes, backupCtx) {
  const expectedByKey = buildExpectedByKey(buildEffectiveConfiguration(systemParsed, systemKey));
  const envPath = path.join(appPath, 'env.template');

  if (createEnvTemplateIfMissing(envPath, expectedByKey, dryRun, changes, systemParsed, backupCtx)) {
    return true;
  }
  ...
  const { output, mergeChanged, pruneChanged } = mergeAndPruneEnvTemplate(
    fs.readFileSync(envPath, 'utf8'),
    expectedByKey
  );
  if (!mergeChanged && !pruneChanged) {
    return false;
  }

  persistEnvTemplate(envPath, output, dryRun, backupCtx);
  if (mergeChanged) {
    changes.push('Repaired env.template (KV_* names and path-style kv:// values)');
  }
  if (pruneChanged) {
    changes.push('Commented stale env.template KV_* lines (not in system authentication)');
  }
  return true;
}
