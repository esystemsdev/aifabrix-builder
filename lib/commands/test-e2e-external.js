...
const { buildMinimalUploadSyncOptions, isQuietMinimalSync } = require('../utils/upload-sync-options');
 ...
 * @returns {Promise<{ authConfig: Object, dataplaneUrl: string, environment?: string }|null>}
 */
async function syncLocalIfRequested(systemKey, options) {
  ...
  const uploadOpts = buildMinimalUploadSyncOptions(options);
  const uploadCtx = await uploadExternalSystem(systemKey, uploadOpts);
  if (uploadCtx?.uploadSkipped !== true && !isQuietMinimalSync(uploadOpts)) {
 ...
 */
/**
 * Resolve integration folder, system key, and datasource keys for system-level tests.
 * @param {string} externalSystem
 * @returns {{ systemKey: string, keys: string[] }}
 */
function resolveExternalIntegrationContext(externalSystem) {
...
async function runTestE2EForExternalSystem(externalSystem, options = {}) {
  ...
  } else if (options.authConfig && options.dataplaneUrl) {
    ({ authConfig, dataplaneUrl } = options);
  } else {
  ...
  const opts = {
    ...
    authConfig,
    dataplaneUrl,
    quietPollUi: options.quietPollUi === true,
    onPollProgress: options.onPollProgress
  };
  for (const key of keys) {
    if (typeof options.onDatasourceProgress === 'function') {
      options.onDatasourceProgress({
        key,
