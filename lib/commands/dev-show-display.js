...
function logConfigurationSection(p) {
  ...
  logRow('Env Config', p.envConfigPath);
  if (!p.hasRemote) {
...
async function loadDevShowData(devIdStr) {
  ...
  const formatRaw = await config.getFormat();
  ...
  const envConfigPath = await config.getAifabrixEnvConfigPath();
  const remoteServer = await config.getRemoteServer();
  const dockerEndpoint = await config.getDockerEndpoint();
  const dockerTlsSkip = Boolean(await config.getDockerTlsSkipVerify());
  const mutagenFolder = await config.getUserMutagenFolder();
  const syncSshUser = await config.getSyncSshUser();
...
function renderDevShow(data) {
  ...
  logConfigurationSection({
    ...
    envConfigPath: data.envConfigPath,
