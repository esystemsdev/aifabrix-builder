...
function logConfigurationSection(p) {
  ...
  logRow('Platform mode', p.setupPlatformMode);
  logRow('Traefik', p.traefikEnabled ? 'on' : 'off');
...
async function loadDevShowData(devIdStr) {
  ...
  const [
    environment, tlsEnabled, controller, formatRaw, useScoped, secretsRaw, setupPlatformMode, traefikEnabled,
    ...
    config.getSetupPlatformMode(), config.getTraefikEnabled(),
...
function renderDevShow(data) {
  ...
  logConfigurationSection({
    ...
    setupPlatformMode: data.setupPlatformMode,
    traefikEnabled: data.traefikEnabled,
