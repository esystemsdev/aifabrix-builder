 ...
 * Skips record_ref: expressions. Returns first segment of each path for top-level schema pruning.
 *
 * @param {Object} attributes - fieldMappings.attributes object
 * @returns {{ paths: string[], topLevelKeys: Set<string> }}
 */
function parsePathsFromExpressions(attributes) {
  const paths = [];
  const topLevelKeys = new Set();
  if (!attributes || typeof attributes !== 'object') return { paths, topLevelKeys };
 ...
 * properties not referenced by any attribute expression path.
 *
 * @param {Object} parsed - Parsed datasource (mutated)
 * @param {string[]} changes - Array to append change descriptions to
...
function repairMetadataSchemaFromAttributes(parsed, changes) {
  const { topLevelKeys } = parsePathsFromExpressions(parsed?.fieldMappings?.attributes ?? {});
  ...
  const toRemove = Object.keys(props).filter(k => topLevelKeys.size > 0 && !topLevelKeys.has(k));
