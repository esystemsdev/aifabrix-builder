 ...
 *
 * Registers or rotates dataplane app in dev, runs it locally, then deploys.
 ...
 * register or rotate (if already registered), run dataplane locally, then deploy.
 *
 * @async
...
async function handleUpDataplane(options = {}) {
  ...
  logger.log(chalk.blue('Starting up-dataplane (register/rotate, run, deploy dataplane in dev)...\n'));
  ...
  const imageOverride = options.image || (options.registry ? buildDataplaneImageRef(options.registry) : undefined);
  const runOpts = { image: imageOverride, registry: options.registry, registryMode: options.registryMode, skipEnvOutputPath: true };
  const deployOpts = { imageOverride, image: imageOverride, registryMode: options.registryMode };

  // Auto deployment not done yet in controller; in future comment this line out
  logger.log(chalk.blue('Starting dataplane locally...'));
  await app.runApp('dataplane', runOpts);

  await app.deployApp('dataplane', deployOpts);

  logger.log(chalk.green('\n✓ up-dataplane complete. Dataplane is registered, running, and deployed in dev.'));
}
