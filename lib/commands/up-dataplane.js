...
async function resolveUpDataplaneAuthContext(options, environmentKey) {
  ...
  if (options.allowNonDev !== true) {
    assertDevEnvironmentForDataplane(cfg);
  }
  logger.log(formatSuccessLine(`Logged in (environment: ${environmentKey})`));
  return { controllerUrl, authConfig };
}
...
async function deployDataplaneToController(options) {
  ...
  const deployOpts = {
    ...
    image: imageOverride,
    registryMode: options.registryMode,
    envKey: options.envKey ? String(options.envKey).trim().toLowerCase() : undefined,
...
function computeDataplanePortOverride(runEnvKey, developerIdNum) {
  if (runEnvKey !== 'dev' && runEnvKey !== 'tst') {
    return undefined;
  }
  const baseHostPort = 3001 + (Number.isFinite(developerIdNum) ? developerIdNum : 0) * 100;
  return runEnvKey === 'dev' ? baseHostPort + 10 : baseHostPort + 20;
}
...
async function computeDataplaneRunOverrides(options) {
  const runEnvKey = options.runEnvKey ? String(options.runEnvKey).trim().toLowerCase() : undefined;
  ...
  const portOverride = computeDataplanePortOverride(runEnvKey, developerIdNum);
  return { runEnvKey, portOverride };
}
...
async function handleUpDataplane(options = {}) {
  ...
  const environmentKey =
    options && options.envKey
      ? String(options.envKey).trim().toLowerCase()
      : await resolveEnvironment();
  const { controllerUrl, authConfig } = await resolveUpDataplaneAuthContext(options, environmentKey);
  ...
  await deployDataplaneToController({ ...options, envKey: environmentKey });
  logger.log('');

  const { runEnvKey, portOverride } = await computeDataplaneRunOverrides(options);
