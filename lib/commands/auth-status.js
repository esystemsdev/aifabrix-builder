...
/**
 * Check and validate client token (refreshes when expired if client credentials exist).
 * @async
 * @param {string} controllerUrl - Controller URL
 * @param {string} environment - Environment key
 * @returns {Promise<Object|null>} Token validation result or null
...
async function checkClientToken(controllerUrl, environment) {
  return tryResolveClientTokenAuth(controllerUrl, environment, validateClientToken);
}
 ...
 * @param {string|undefined} [dataplaneInfo.dataplaneVersion]
 * @param {string|undefined} [dataplaneInfo.minBuilderCliVersion]
 * @param {string} [dataplaneInfo.cliVersion]
 * @param {boolean} [dataplaneInfo.compatible]
 */
function displayStatus(controllerUrl, environment, tokenInfo, dataplaneInfo) {
  ...
  if (tokenInfo.authenticated && dataplaneInfo) {
    ...
    displayDataplaneVersionSection({
      connected: dataplaneInfo.connected,
      dataplaneVersion: dataplaneInfo.dataplaneVersion,
      minBuilderCliVersion: dataplaneInfo.minBuilderCliVersion,
      cliVersion: dataplaneInfo.cliVersion,
      compatible: dataplaneInfo.compatible
    });
    if (dataplaneInfo.minBuilderCliVersion && dataplaneInfo.compatible === false) {
      displayCliUpgradeRequired({
        required: dataplaneInfo.minBuilderCliVersion,
        installed: dataplaneInfo.cliVersion
      });
    }
  }
...
/**
 * Resolve the dataplane block for auth status (URL, reachability, versions,
 * compatibility). Returns null when no dataplane is discovered.
 *
 * @async
 * @function buildDataplaneInfo
 * @param {string} controllerUrl
 * @param {string} environment
 * @param {Object} authConfig - Bearer auth config built from a valid token
 * @returns {Promise<Object|null>}
 */
async function buildDataplaneInfo(controllerUrl, environment, authConfig) {
  const dataplaneUrl = await resolveDataplaneUrlSilent(controllerUrl, environment, authConfig);
  if (!dataplaneUrl) {
    return { url: null, connected: false };
  }
  const connected = await checkDataplaneHealth(dataplaneUrl, 5000);
  const versionInfo = connected
    ? await refreshDataplaneVersionInfo(controllerUrl, dataplaneUrl)
    : await loadCachedVersionInfo(controllerUrl);
  return {
    url: dataplaneUrl,
    connected,
    dataplaneVersion: versionInfo.dataplaneVersion,
    minBuilderCliVersion: versionInfo.minBuilderCliVersion,
    cliVersion: versionInfo.cliVersion,
    compatible: versionInfo.compatible
  };
}
 ...
 * @param {boolean} [options.validate] - If true, exit with code 1 when not authenticated, or code 3 when CLI is older than dataplane minimum
 * @returns {Promise<void>} Resolves when status is displayed
 */
/**
 * Prefer a working device token; fall back to environment client credentials when the
 * stored device token is expired or rejected (common in long-lived dev VMs).
 *
 * @async
 * @param {string} controllerUrl
 * @param {string} environment
 * @returns {Promise<Object|null>}
 */
async function resolveAuthenticatedTokenInfo(controllerUrl, environment) {
  const deviceResult = await checkDeviceToken(controllerUrl);
  if (deviceResult && deviceResult.authenticated) {
    return deviceResult;
  }
  const clientResult = await checkClientToken(controllerUrl, environment);
  if (clientResult && clientResult.authenticated) {
    return clientResult;
  }
  return deviceResult || clientResult || null;
}
...
async function handleAuthStatus(options = {}) {
  ...
  const refreshedCount = await refreshExpiredClientTokensWherePossible(environment, controllerUrl);
  const tokenInfo = await resolveAuthenticatedTokenInfo(controllerUrl, environment);
  ...
  if (tokenInfo && tokenInfo.authenticated && tokenInfo.token) {
    ...
    dataplaneInfo = await buildDataplaneInfo(controllerUrl, environment, authConfig);
  }

  displayStatus(controllerUrl, environment, tokenInfo, dataplaneInfo);
  await displayStoredClientTokensSection(environment, controllerUrl, refreshedCount);

  if (options.validate) {
    if (!tokenInfo || !tokenInfo.authenticated) {
      process.exit(EXIT_NOT_AUTHENTICATED);
    }
    if (dataplaneInfo && dataplaneInfo.minBuilderCliVersion && dataplaneInfo.compatible === false) {
      process.exit(EXIT_CLI_VERSION_INCOMPATIBLE);
    }
