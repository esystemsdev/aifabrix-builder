...
async function getExistingOpenApiKeys(dataplaneUrl, systemKey, authConfig) {
  ...
  if (!res || res.success !== true) {
    const msg = res && typeof res.formattedError === 'string'
      ? res.formattedError
      : (res && typeof res.error === 'string' ? res.error : 'Failed to list OpenAPI files');
    throw new Error(msg);
  }
  const items =
    (res && res.data && Array.isArray(res.data.data) ? res.data.data : null) ||
    (res && res.data && Array.isArray(res.data.items) ? res.data.items : null) ||
