...
function exitCodeFromDatasourceTestRunEnvelope(env, options) {
  ...
  const scope = analyzeCapabilityScope(env, options.capabilityKey);
...
function finalizeDatasourceTestE2ELegacyPath(data, options) {
  ...
  logE2eCapabilityFocusFromEnvelope(data.datasourceTestRun, options.capabilityKey);
  const env = data.datasourceTestRun;
  if (env) {
    emitCapabilityScopeDiagnostics(env, { requestedCapabilityKey: options.capabilityKey });
...
function displayDatasourceTestE2EEnvelopeResults(datasourceKey, env, options) {
  ...
  logE2eCapabilityFocusFromEnvelope(env, options.capabilityKey);
}
