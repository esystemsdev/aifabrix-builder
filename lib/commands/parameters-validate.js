...
async function handleParametersValidate(options = {}) {
  ...
  try {
    ...
    logger.log(chalk.red(`✗ Could not load infra parameter catalog: ${e.message}`));
    return { valid: false };
  }

  const reqGen = validateCatalogRequiredGenerators(catalog.data);
  if (!reqGen.valid) {
    logger.log(chalk.red('✗ Catalog requiredForLocal / generator issues:'));
  ...
  if (!kv.valid) {
    logger.log(chalk.red('✗ env.template kv:// keys not covered by infra.parameter.yaml:'));
    kv.errors.forEach((err) => logger.log(chalk.yellow(`  • ${err}`)));
    return { valid: false };
  }

  logger.log(chalk.green('✓ parameters validate: catalog OK; workspace kv:// keys covered.'));
  return { valid: true };
}
