...
const { repairTestPayload } = require('./repair-datasource-test-payload');
...
/**
 * @param {Object} datasourceConfig - Datasource config (mutated)
 */
function normalizePortalInput(datasourceConfig) {
  if (!Array.isArray(datasourceConfig.portalInput)) return;
  datasourceConfig.portalInput = datasourceConfig.portalInput.filter(item => {
    if (!item || typeof item !== 'object') return false;
    if (!item.name || !item.field || !item.label) return false;
 ...
 * @param {Object} datasourceConfig - Datasource config (mutated)
 */
function normalizeCipOutputModes(datasourceConfig) {
  const operations = datasourceConfig.execution?.cip?.operations;
  if (!operations) return;
  for (const operation of Object.values(operations)) {
    if (!operation || !Array.isArray(operation.steps)) continue;
 ...
 * @param {Object} datasourceConfig - Datasource config (mutated)
 */
function normalizeWizardTestPayload(datasourceConfig) {
  const attrs = datasourceConfig.fieldMappings?.attributes;
  if (!datasourceConfig.metadataSchema || !attrs || Object.keys(attrs).length === 0) {
    return;
  }
  repairTestPayload(datasourceConfig, []);
}
...
function normalizeDatasourceConfig(datasourceConfig) {
  ...
  normalizePortalInput(datasourceConfig);
  normalizeCipOutputModes(datasourceConfig);
  normalizeWizardTestPayload(datasourceConfig);
  return datasourceConfig;
}
