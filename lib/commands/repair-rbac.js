 ...
 * @returns {{ rbacMigratedFromSystem: boolean, rbacMergeUpdated: boolean }}
 */
function maybeRepairRbac(appPath, systemFilePath, systemParsed, datasourceFiles, options) {
  ...
  const rbacMigratedFromSystem = migrateSystemRbacIntoRbacFile(appPath, systemFilePath, systemParsed, {
    dryRun,
    changes,
    backupCtx
  });
