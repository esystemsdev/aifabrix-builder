 ...
 * and persist ``.aifabrix/config.yaml`` (or ``AIFABRIX_CONFIG``). Does not remove ``controller`` or
 * ``developer-id``. Builder dirs (keycloak, miso-controller, dataplane) are cleaned separately.
 *
  ...
  logger.log(
    chalk.blue(
      `--force: cleared ${deviceCleared} device token(s) and ${clientCleared} client token(s); ` +
        'environment set to dev (run aifabrix login after platform is up)'
    )
  );
}
