 ...
 * set ``controller`` and ``platform-controller`` to the default URL (or ``defaultControllerUrl`` when
 * provided), and persist config. Does not change ``developer-id``. Pass ``clearTokens: false`` to
 * keep device/client tokens (setup when already logged in to the platform controller).
...
async function applyUpPlatformForceConfig(opts = {}) {
  ...
  let clientCleared = 0;
  if (clearTokens) {
    deviceCleared = await config.clearAllDeviceTokens();
    clientCleared = await config.clearAllClientTokens();
  }
  await config.setCurrentEnvironment('dev');
  const defaultControllerUrl =
    opts.defaultControllerUrl && typeof opts.defaultControllerUrl === 'string'
      ? String(opts.defaultControllerUrl).replace(/\/+$/, '')
      : await getDefaultControllerUrl();
  await config.setControllerUrl(defaultControllerUrl);
  await config.setPlatformControllerUrl(defaultControllerUrl);
 ...
 */
async function ensureAppFromTemplate(appName, opts = {}) {
 ...
 * @returns {Promise<void>}
 */
async function prepareUrlsLocalRegistryForUpPlatform(opts = {}) {
  const templateOpts = { silent: Boolean(opts.silent) };
  await ensureAppFromTemplate('keycloak', templateOpts);
  await ensureAppFromTemplate('miso-controller', templateOpts);
  await ensureAppFromTemplate('dataplane', templateOpts);
