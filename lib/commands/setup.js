...
const setupPlatformModePrompts = require('./setup-prompts-platform-mode');
const { flagsForSetupPlatformMode } = require('../core/setup-platform-mode');
const setupModes = require('./setup-modes');
const { syncPlatformControllerUrlsInConfig } = require('./setup-platform-auth');
const installationLog = require('../utils/installation-log');
const { beginSetupCommandFlow, endSetupCommandFlow } = require('./setup-run-context');
...
async function applySetupPlatformMode(options) {
  const mode = await setupPlatformModePrompts.promptSetupPlatformMode(options);
  await config.setSetupPlatformMode(mode);
  const derived = flagsForSetupPlatformMode(mode);
  // These are the user gates that control full-mode behavior.
  const cfg = await config.getConfig();
  cfg.traefik = derived.traefik;
  cfg.useEnvironmentScopedResources = derived.useEnvironmentScopedResources;
...
async function runFreshInstallFlow(options) {
  ...
  await applySetupPlatformMode(options);
...
async function runExistingInfraFlow(assumeYes) {
  ...
  // Re-install / wipe-data can change platform topology; re-prompt so the next up-platform matches intent.
  if (mode === MODE.REINSTALL || mode === MODE.WIPE_DATA) {
    await applySetupPlatformMode({});
  }
