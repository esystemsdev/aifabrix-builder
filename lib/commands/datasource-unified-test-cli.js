...
const { maybeVerifyAuditAfterE2E } = require('../datasource/audit-evidence-run');
const {
  ...
  if (unifiedModes && data.datasourceTestRun) {
    const exitCode = finalizeUnifiedValidationResult(
      ...
      },
      {
        ...
        requireCert: options.requireCert,
        debug: options.debug,
        requestedCapabilityKey: options.capabilityKey,
...
async function runDatasourceTestE2ECliAction(datasourceKey, options) {
  ...
  const finalExit = await maybeVerifyAuditAfterE2E(exitCode, datasourceKey, envelope, options);
  await afterUnifiedValidationCertSync(
    finalExit,
    datasourceKey,
    options,
...
async function e2eTestCommandAction(datasourceKey, capabilityKey, options) {
  ...
  const mergedOptions = {
    ...optObj,
    capabilityKey: capFromArg || undefined
  };
  try {
      ...
      await runDatasourceValidationWatchLoop({
        ...
        runOnce: async() => {
          try {
            ...
            const finalExit = await maybeVerifyAuditAfterE2E(
              r.exitCode,
              datasourceKey,
              r.envelope,
              mergedOptions
            ...
            return { exitCode: finalExit, envelope: r.envelope };
          } catch (err) {
