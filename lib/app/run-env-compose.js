...
const { applyAdminSecretsPlatformOverlay } = require('../core/admin-secrets-env-overlay');

/**
 * Merge app env with admin-secrets; overlay ensures platform keys come from admin file.
 * @param {Object.<string, string>} merged
 * @param {Object.<string, string>} adminObj
 */
function mergeRunEnvWithAdminOverlay(merged, adminObj) {
  applyAdminSecretsPlatformOverlay(merged, adminObj);
}

async function applyOnboardingPlatformModeIfNeeded(appName, merged) {
  if (appName !== 'miso-controller') {
    return;
  }
  const mode = await config.getSetupPlatformMode();
  merged.ONBOARDING_FULL_PLATFORM = mode === 'full' ? 'true' : 'false';
}
...
/**
 * Read POSTGRES_PASSWORD from pgpass-style files beside the active infra compose
 * (`resolveInfraStatePaths()`). Tries legacy `pgpass`, then `.pgpass.bootstrap` written at `up-infra`
 * so db-init matches the password Postgres was started with when admin-secrets later drifted.
 * @param {number|string} developerId - Developer ID
 * @returns {Promise<string|undefined>} Password or undefined
 */
async function readPostgresPasswordFromPgpass(developerId) {
  const { resolveInfraStatePaths, PGPASS_BOOTSTRAP_BASENAME } = require('../infrastructure/helpers');
  const { infraDir } = resolveInfraStatePaths(developerId);
  const candidates = ['pgpass', PGPASS_BOOTSTRAP_BASENAME];
  for (const name of candidates) {
    const pgpassPath = path.join(infraDir, name);
 ...
 * @returns {Function}
 */
function resolveEnsureAdminSecretsFn() {
  const infra = require('../infrastructure');
  return typeof infra.ensureAdminSecrets === 'function'
...
/**
 * @param {Object} merged
 * @param {number|string|undefined} developerId
 * @returns {Promise<void>}
 */
async function applyPostgresPasswordFromPgpass(merged, developerId) {
...
/**
 * @param {string} runEnvPath
 * @param {string} runEnvAdminPath
 * @param {Object} appOnly
 * @param {Object} dbInitOnly
 * @returns {Promise<void>}
 */
async function writeRunEnvFiles(runEnvPath, runEnvAdminPath, appOnly, dbInitOnly) {
  const toContent = adminSecrets.envObjectToContent;
  await fs.writeFile(runEnvPath, toContent(appOnly), { mode: 0o600 });
  await fs.writeFile(runEnvAdminPath, toContent(dbInitOnly), { mode: 0o600 });
}
 ...
 * When `pgpass` or `.pgpass.bootstrap` exists beside the active infra compose, POSTGRES_PASSWORD is taken
 * from it so db-init matches Postgres.
 * @async
 * @param {string} appName - Application name
...
async function buildMergedRunEnvAndWrite(appName, appConfig, devDir, developerId, scopeOpts = null) {
  ...
  await resolveEnsureAdminSecretsFn()();
  ...
  mergeRunEnvWithAdminOverlay(merged, adminObj);
  await applyOnboardingPlatformModeIfNeeded(appName, merged);
  await applyPostgresPasswordFromPgpass(merged, developerId);
  ...
  await writeRunEnvFiles(
    runEnvPath,
    runEnvAdminPath,
    buildAppOnlyEnv(merged),
    buildDbInitOnlyEnv(merged)
  );
