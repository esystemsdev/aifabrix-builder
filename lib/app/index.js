...
/**
 * Ensures secrets from env.template. On EACCES (e.g. secrets path under a read-only tree), uses ~/.aifabrix/secrets.local.yaml.
...
async function ensureSecretsFromNewAppEnvTemplate(envTemplatePath) {
  try {
    ...
    if (err.code === 'EACCES' || (err.message && String(err.message).includes('EACCES'))) {
      const { getAifabrixHome } = require('../utils/paths');
      await secretsEnsure.ensureSecretsFromEnvTemplate(envTemplatePath, {
        preferredFilePath: path.join(getAifabrixHome(), 'secrets.local.yaml')
      });
      return;
    }
    throw err;
