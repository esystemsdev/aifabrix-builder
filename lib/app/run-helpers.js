...
const { resolveEnvOutputPath } = require('../utils/env-copy');
...
async function checkPrerequisites(appName, appConfig, debug = false, skipInfraCheck = false, runOptions = {}) {
  let imageName;
  let imageTag;
  const imageOverride = runOptions?.image;
  if (imageOverride) {
    ...
    imageTag = parsed ? parsed.tag : (appConfig.image?.tag || 'latest');
  } else {
    imageName = composeGenerator.getImageName(appConfig, appName);
    imageTag = appConfig.image?.tag || 'latest';
  }
...
/**
 * Resolve and write .env for run: in-memory resolution, write only to envOutputPath or temp.
 * No .env written to builder/ or integration/. Returns path for compose env_file.
 *
 * @async
 * @param {string} appName - Application name
 * @param {Object} appConfig - Application configuration
 * @param {boolean} [skipEnvOutputPath=false] - When true, do not use envOutputPath (e.g. up-miso/up-dataplane)
 * @returns {Promise<string>} Path to .env file (envOutputPath or temp)
 */
async function resolveAndWriteRunEnv(appName, appConfig, skipEnvOutputPath = false) {
  let envOutputPath = null;
  if (!skipEnvOutputPath && appConfig?.build?.envOutputPath) {
    const configPath = pathsUtil.resolveApplicationConfigPath(pathsUtil.getBuilderPath(appName));
    envOutputPath = resolveEnvOutputPath(appConfig.build.envOutputPath, configPath);
  }
  logger.log(chalk.blue('Resolving .env for Docker...'));
  return await secretsEnvWrite.resolveAndWriteEnvFile(appName, {
    envOutputPath,
    environment: 'docker',
    secretsPath: null,
    force: false
...
/**
 * Prepares environment: resolve .env in memory (write only to envOutputPath or temp), generate Docker Compose.
 * No .env written to builder/ or integration/.
 *
 * @async
 * @param {string} appName - Application name
 * @param {Object} appConfig - Application configuration
...
async function prepareEnvironment(appName, appConfig, options) {
  ...
  const envFilePath = await resolveAndWriteRunEnv(appName, appConfig, skipEnvOutputPath);
  const composeOptions = { ...options, envFilePath };
