...
async function writeEnvOutputIfConfigured(appName, appConfig, runEnvPath, options) {
  ...
  if (options.reload) {
    const content = await fs.readFile(runEnvPath, 'utf8');
    ...
    const localContent = await secrets.generateEnvContent(appName, null, 'local', false);
    await fs.writeFile(outputPath, localContent, { mode: 0o600 });
