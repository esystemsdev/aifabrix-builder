...
function assertDeployNotProtectionScope(appName) {
  const { isProtectionScope } = require('../protection/scope');
  if (!isProtectionScope(appName)) {
    return;
  }
  const { formatDeployProtectionNotImplementedTTY } = require('../protection/protection-display');
  logger.error(formatDeployProtectionNotImplementedTTY());
  process.exit(1);
}

/**
 * Run external, fallback, or standard deployment paths.
 * @param {string} appName
 * @param {Object} options
 * @returns {Promise<{ result: Object, usedExternalDeploy: boolean, controllerUrl?: string }>}
 */
async function executeDeployPipeline(appName, options) {
  ...
  if (fallback.result) {
    return { result: fallback.result, usedExternalDeploy: fallback.usedExternalDeploy };
  }
  const { result, controllerUrl } = await executeStandardDeployment(appName, options);
  return { result, usedExternalDeploy: false, controllerUrl };
}
...
async function deployApp(appName, options = {}) {
  ...
  try {
    assertDeployNotProtectionScope(appName);
    validateAppName(appName);
    const pipeline = await executeDeployPipeline(appName, options);
    controllerUrl = pipeline.controllerUrl ?? null;
    usedExternalDeploy = pipeline.usedExternalDeploy;
    return { result: pipeline.result, usedExternalDeploy };
  } catch (error) {
