 ...
 */
const { collectDeveloperVolumeIdSuffixes } = require('../utils/developer-volume-id');
...
/**
 * Docker volume name candidates for an app (handles dev6 vs dev06 naming drift).
 *
 * @param {string} appName
 * @param {string|number} developerId
 * @returns {string[]}
 */
function getAppVolumeNameCandidates(appName, developerId) {
  const idNum = typeof developerId === 'string' ? parseInt(developerId, 10) : developerId;
  if (idNum === 0) {
    return [`aifabrix_${appName}_data`, `aifabrix_dev0_${appName}_data`];
  }
  const suffixes = collectDeveloperVolumeIdSuffixes(developerId);
  return suffixes.map((suffix) => `aifabrix_dev${suffix}_${appName}_data`);
}
