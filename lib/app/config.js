 ...
 * Aligns with schema authenticationVariablesByMethod; security keys use kv:// format.
...
function generateExternalSystemEnvTemplate(config, appName) {
  ...
  const lines = [
    ...
    '# Use kv://<systemKey>/<key> format per authentication schema',
    ''
  ];

  const kv = (key) => `kv://${systemKey}/${key}`;
  if (authType === 'oauth2' || authType === 'aad') {
    lines.push(`CLIENT_ID=${kv('client-id')}`);
    lines.push(`CLIENT_SECRET=${kv('client-secret')}`);
    lines.push('TOKEN_URL=https://api.example.com/oauth/token');
  } else if (authType === 'apikey') {
    lines.push(`API_KEY=${kv('api-key')}`);
  } else if (authType === 'basic') {
    lines.push(`USERNAME=${kv('username')}`);
    lines.push(`PASSWORD=${kv('password')}`);
  } else if (authType === 'queryParam') {
    lines.push(`PARAM_VALUE=${kv('param-value')}`);
  } else if (authType === 'oidc') {
    lines.push('# OIDC: variables only (openIdConfigUrl, clientId); no security keys');
  } else if (authType === 'hmac') {
    lines.push(`SIGNING_SECRET=${kv('signing-secret')}`);
  }
