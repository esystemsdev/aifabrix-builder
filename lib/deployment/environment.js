...
const { fetchDeploymentStatusById, enrichPollTerminalError } = require('./environment-poll-status');
 ...
 * @returns {Promise<Object|null>} Deployment record or null when still in progress
 * @throws {Error} On terminal poll errors (e.g. environment or deployment not found)
 */
async function getDeploymentStatusById(controllerUrl, envKey, deploymentId, apiAuthConfig) {
  return fetchDeploymentStatusById(
 ...
 */
function logPollAttemptProgress(attempt, maxAttempts, deployment) {
...
/**
 * @param {Error} error
 * @param {string} validatedEnvKey
 * @throws {Error}
 */
function handlePollIterationError(error, validatedEnvKey) {
  const msg = error?.message || '';
  const isDeploymentFailed =
    msg.includes('Environment deployment failed') ||
    (msg.includes('failed') && !error.pollTerminal);
  if (error.pollTerminal || isDeploymentFailed) {
    throw enrichPollTerminalError(validatedEnvKey, error);
  }
  logger.log(chalk.yellow(`   ⚠ Status check failed (will retry): ${msg}`));
}
...
async function pollEnvironmentStatus(deploymentId, controllerUrl, envKey, authConfig, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      logPollAttemptProgress(attempt, maxAttempts, deployment);
      const statusResult = processDeploymentStatusResponse(deployment, validatedEnvKey);
      if (statusResult) {
        return statusResult;
      }
    } catch (error) {
      handlePollIterationError(error, validatedEnvKey);
    }
  }
