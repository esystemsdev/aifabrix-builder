 ...
 * @returns {string}
 */
function buildUploadPipelineSpinnerLabel(systemKey, opts = {}) {
  if (opts.syncMode === true) {
    return 'Checking config on dataplane…';
  }
 ...
 * @returns {string}
 */
function buildUploadPipelineProgressText(systemKey, opts = {}) {
  return formatProgress(buildUploadPipelineSpinnerLabel(systemKey, opts));
}
...
async function runWithUploadPipelineSpinner(work, opts = {}) {
  ...
  if (!shouldUseUploadPipelineSpinner(opts)) {
    logger.log(buildUploadPipelineProgressText(opts.systemKey, opts));
    return work();
  }

  const ora = require('ora');
  const label = buildUploadPipelineSpinnerLabel(opts.systemKey, opts);
  const spinner = ora({
