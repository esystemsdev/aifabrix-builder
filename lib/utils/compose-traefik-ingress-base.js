...
function resolveTraefikRouterHost(hostTemplate, developerIdRaw, remoteServer) {
  const usesRemote = templateUsesRemoteHostPlaceholder(hostTemplate);
  const hasRemote = Boolean(remoteServer && String(remoteServer).trim());
  if (usesRemote && !hasRemote) {
    return 'localhost';
  }
  return expandFrontDoorHostPlaceholders(hostTemplate, developerIdRaw, remoteServer);
}
...
function buildTraefikIngressBase(config, devId, scopeOpts, remoteServer) {
  ...
  const host = resolveTraefikRouterHost(frontDoor.host, devId, remoteServer);
...
module.exports = {
  ...
  templateUsesRemoteHostPlaceholder,
  resolveTraefikRouterHost,
