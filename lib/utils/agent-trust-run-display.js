...
function logSystemTrustTable(list, opts = {}) {
  ...
  if (showCache) {
    logger.log(
      chalk.white('  Datasource                    Trust                  Conf.   Status     Cache')
    );
  ...
  for (const row of list) {
    ...
    const cacheSuffix = showCache ? `   ${trustCacheLabel(tr).padEnd(6)}` : '';
    logger.log(`  ${key}  ${trustGlyph(tr && tr.trustDecision)} ${td}  ${conf}   ${vs}${cacheSuffix}`);
  }
}
