...
function loadExternalSystemSchema() {
  ...
  if (!nodeFsLib.nodeFs().existsSync(schemaPath)) {
    throw new Error(`External system schema not found: ${schemaPath}`);
  }

  const schemaContent = nodeFsLib.nodeFs().readFileSync(schemaPath, 'utf8');
  let schema;
...
function readAndParseFileContent(filePath, content) {
  ...
  if (!fileContent) {
    if (!nodeFsLib.nodeFs().existsSync(filePath)) {
      throw new Error(`File not found: ${filePath}`);
    }
    fileContent = nodeFsLib.nodeFs().readFileSync(filePath, 'utf8');
  }
