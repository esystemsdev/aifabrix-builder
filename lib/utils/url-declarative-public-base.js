...
function localhostFrontDoorBaseForMissingRemote(opts) {
  const { profile, developerIdNum } = opts || {};
  const port = hostPortForProfile(profile, 3000, developerIdNum);
...
function buildTraefikPublicBaseIfApplicable(opts) {
  ...
  if (templateUsesRemoteHostPlaceholder(hostTemplate) && !String(remoteServer || '').trim()) {
    // Local full-platform (no remote domain): use the single localhost front door port (3000 + devId×100).
    return localhostFrontDoorBaseForMissingRemote({
      profile,
      developerIdNum
    });
  }
