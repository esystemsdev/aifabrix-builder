 ...
 * @param {number} opts.developerIdNum
 * @param {boolean} [opts.infraTlsEnabled] - `tlsEnabled` from ~/.aifabrix/config.yaml (`up-infra --tls`); when true, Traefik front-door public URLs use https even if application.yaml has `frontDoorRouting.tls: false`, except loopback hostnames stay **http**
 * @param {boolean} [opts.pathActive] - traefik ∧ frontDoorRouting.enabled; required for Traefik host branch (plan 124)
 * @param {boolean} [opts.declarativePublicUrlsUseLocalhost] - Use localhost + **published** port (`port + dev*100`, same as compose host mapping / application.yaml comment); scheme is always **http** (loopback, regardless of `tlsEnabled`).
 * @returns {string}
 */
