...
/**
 * Scheme for `declarativePublicUrlsUseLocalhost` bases: align with remote dev TLS when `remote-server` is https.
 * @param {Object} opts - same subset as {@link computePublicUrlBaseString}
 * @returns {'http'|'https'}
 */
function schemeForDeclarativeLocalhostPublicBase(opts) {
  if (opts.infraTlsEnabled) {
    return 'https';
  }
  const rs = String(opts.remoteServer || '').trim();
  if (/^https:\/\//i.test(rs)) {
    return 'https';
  }
  return 'http';
 ...
 * @param {boolean} [opts.declarativePublicUrlsUseLocalhost] - Use localhost + **published** port (`port + dev*100`, same as compose host mapping / application.yaml comment); scheme is https when `infraTlsEnabled` or `remote-server` is an `https://` URL, else http.
 * @returns {string}
 */
function computePublicUrlBaseString(opts) {
  ...
  if (opts.declarativePublicUrlsUseLocalhost) {
    const hostPort = publishedHostPort(listenPort, developerIdNum);
