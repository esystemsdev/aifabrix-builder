 ...
 * @param {boolean} [opts.declarativePublicUrlsUseLocalhost] - Use **localhost** as authority (direct / `--no-proxy`); port follows {@link resolveHostPortForDeclarativePublic} — same **+10** rules as the non-localhost branch for **local** profile (current app vs cross-app), **docker** unchanged (published). Scheme is always **http** (loopback, regardless of `tlsEnabled`).
 * @returns {string}
 */
