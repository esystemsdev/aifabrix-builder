 ...
 */
async function syncWrittenBuilderEnvToOutputPath(envPath, outputPath, envOutputPathLabel, copyOptions = {}) {
...
async function writeLocalEnvToOutputPath(outputPath, appName, secretsPath, envOutputPathLabel, extraOpts = {}) {
  ...
  if (!extraOpts.freshEnv && fs.existsSync(outputPath)) {
    const existingContent = fs.readFileSync(outputPath, 'utf8');
    const localMap = parseEnvContentToMap(localEnvContent);
    toWrite = mergeEnvMapIntoContent(existingContent, localMap, { appendMissingFromNewMap: true });
  }
  fs.writeFileSync(outputPath, toWrite, { mode: 0o600 });
...
async function copyOrRegenerateEnvForNamedApp(opts) {
  ...
  const localOpts = { appPath: appPathForLocal || undefined, freshEnv: freshEnv === true };
  if (preferLocal) {
    await writeLocalEnvToOutputPath(outputPath, appName, secretsPath, label, localOpts);
    return;
  }
  if (fs.existsSync(envPath)) {
    await syncWrittenBuilderEnvToOutputPath(envPath, outputPath, label, { freshEnv: freshEnv === true });
    return;
  }
  await writeLocalEnvToOutputPath(outputPath, appName, secretsPath, label, localOpts);
}
...
async function processEnvVariables(envPath, variablesPath, appName, secretsPath, copyOptions = {}) {
  ...
  const freshEnv = copyOptions && typeof copyOptions === 'object' && copyOptions.freshEnv === true;
  if (appName) {
