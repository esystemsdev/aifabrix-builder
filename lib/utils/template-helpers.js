 ...
 * @param {Object} config - Final configuration
 */
async function patchEnvTemplateRelativePathKey(appPath, appName) {
  const fsSync = require('fs');
  const envTemplatePath = path.join(appPath, 'env.template');
  if (!fsSync.existsSync(envTemplatePath)) {
    return;
  }
  const { renameAppRelativePathEnvKeyInTemplate } = require('./app-relative-path-env');
  const raw = await fs.readFile(envTemplatePath, 'utf8');
  const patched = renameAppRelativePathEnvKeyInTemplate(raw, appName);
  if (patched !== raw) {
    await fs.writeFile(envTemplatePath, patched, 'utf8');
  }
}
...
async function updateTemplateVariables(appPath, appName, options, config) {
  ...
  try {
    ...
    await patchEnvTemplateRelativePathKey(appPath, appName);
  } catch (error) {
