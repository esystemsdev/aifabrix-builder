...
function getOrderedBuilderDirsForRegistryScan(root, effectiveBuilderDir) {
  ...
  const envOverridesBuilder =
    process.env.AIFABRIX_BUILDER_DIR && String(process.env.AIFABRIX_BUILDER_DIR).trim() !== '';

  if (effectiveBuilderDir && resolvedEffective && resolvedEffective === resolvedLegacy) {
    return [legacyBuilderDir];
  }
  if (envOverridesBuilder && effectiveBuilderDir && resolvedEffective && resolvedEffective !== resolvedLegacy) {
