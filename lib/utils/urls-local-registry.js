 ...
 * @param {Object} merged
 */
function mergePackagesDirsFromKnownRoots(merged) {
  const packageScanRoots = new Set();
  try {
    packageScanRoots.add(pathsUtil.getIntegrationBuilderBaseDir());
  } catch {
    // ignore
  }
  try {
    ...
    if (pr) {
      packageScanRoots.add(pr);
    }
  } catch {
    // ignore
  }
  for (const base of packageScanRoots) {
    if (!base || typeof base !== 'string') {
      continue;
    }
    mergeBuilderDirIntoRegistry(merged, path.join(base, 'packages'));
  }
}
...
function refreshUrlsLocalRegistryFromBuilder(projectRoot) {
  ...
  mergePackagesDirsFromKnownRoots(merged);
  return writeMergedRegistry(merged);
}
