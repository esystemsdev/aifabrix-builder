 ...
 * @returns {string}
 */
function buildValidationPollSpinnerText(envelope, attemptIndex, deadlineMs, subject) {
  const remainingSec = Math.max(0, Math.ceil((deadlineMs - Date.now()) / 1000));
  const label =
    subject && String(subject).trim() ? String(subject).trim() : 'validation run';
  if (!envelope || typeof envelope !== 'object') {
    return `Waiting for ${label}… starting (budget ~${remainingSec}s)`;
  }
  const c =
  ...
  return `Waiting for ${label}… completeness=${c} status=${st} (poll ${attemptIndex + 1} · ~${remainingSec}s left)`;
}
 ...
 * @param {Object} [opts]
 * @param {string} [opts.subject] - Datasource key or label shown in poll text
 * @returns {{ usesSpinner: boolean, onPollProgress: Function, finish: () => void }}
 */
function createValidationPollHandlers(deadlineMs, opts = {}) {
  if (opts.quiet === true) {
    return {
      usesSpinner: false,
      onPollProgress: () => {},
  ...
  if (shouldUseValidationPollSpinner()) {
    ...
    const spinner = ora({
      text: buildValidationPollSpinnerText(null, 0, deadlineMs, subject),
      spinner: 'dots'
    }).start();
    return {
      usesSpinner: true,
      onPollProgress: (envelope, attemptIndex, _meta) => {
        spinner.text = buildValidationPollSpinnerText(envelope, attemptIndex, deadlineMs, subject);
      },
      finish: () => {
        spinner.stop();
      }
  ...
  return {
    ...
    onPollProgress: (envelope, attemptIndex, _meta) => {
      ...
      lastNonTtyLogAt = now;
      logger.log(
        chalk.gray(buildValidationPollSpinnerText(envelope, attemptIndex, deadlineMs, subject))
      );
    },
    finish: () => {}
  };
