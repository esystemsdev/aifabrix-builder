 ...
 * @param {boolean} [ctx.declarativePublicUrlsUseLocalhost] - When `ctx.userCfg` is absent: public bases use localhost + port when true. When `ctx.userCfg` is set: ignored; derived per target app from `applications.<target>.proxy` (`proxy: false` / `af run --no-proxy` ⇒ localhost for `local` and `docker` profiles).
 ...
 * @param {boolean} [ctx.infraTlsEnabled] - When true (`tlsEnabled` in config / `up-infra --tls`), Traefik front-door `url://public` uses https even if application.yaml has `frontDoorRouting.tls: false`, except **loopback** hosts (`localhost`, `127.0.0.1`, `::1`) stay **http**
...
async function expandDeclarativeUrlsInEnvContent(content, ctx) {
  ...
  const registry = refreshUrlsLocalRegistryFromBuilder(pr || pathsUtil.getProjectRoot());
