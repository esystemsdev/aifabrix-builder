 ...
 * @param {string|null|undefined} [ctx.projectRoot] - Optional absolute project root for `builder/<app>/application.yaml` discovery and `urls.local.yaml` refresh; when set, avoids relying on `paths.getProjectRoot()` (stable under cross-test `paths` mocks). When non-empty, registry refresh **does not** append the checkout `cwd/builder` scan pass (avoids merging unrelated apps into `urls.local.yaml` during isolated resolves).
