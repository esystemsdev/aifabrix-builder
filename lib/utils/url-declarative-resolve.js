 ...
 * @param {boolean} [ctx.declarativePublicUrlsUseLocalhost] - When `ctx.userCfg` is absent: public bases use localhost + port when true. When `ctx.userCfg` is set: ignored; derived per target app from `applications.<target>.proxy` (`proxy: false` / `af run --no-proxy` ⇒ **localhost** authority for public bases; **port** still follows local-profile **+10** rules for the current app vs published-only for cross-app—**not** Traefik ingress).
