...
function replaceUrlRefToken(token, ctx, profile, devNum, derivedEnvKey, registry) {
  ...
  const { appKey, listenPort, patternStr, hostTemplate, tls } = resolved;
 ...
 * @param {boolean} [ctx.traefik] - When true and target app has frontDoorRouting.host, public URLs use that template (https by default)
