...
async function generateDockerCompose(appName, appConfig, options) {
  ...
  const misoEnvironment = (s => (s === 'tst' || s === 'pro') ? s : 'dev')((options.env && typeof options.env === 'string') ? options.env.toLowerCase() : 'dev');
  const templateData = {
    ...serviceConfig,
    ...volumesConfig,
    ...networksConfig,
    ...
    containerName,
    misoEnvironment,
    devMountPath: (options.devMountPath && typeof options.devMountPath === 'string') ? options.devMountPath.trim() : null
  };
