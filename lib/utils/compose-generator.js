...
function computeAlignedHealthCheckPath(config, devId, scopeOpts, remoteServer, readinessSuffix) {
  ...
  const traefik = buildTraefikConfig(config, devId, scopeOpts, remoteServer);
 ...
 * @param {Object|null} scopeOpts - Env-scoped Traefik path (effectiveEnvironmentScopedResources, runEnvKey)
 * @param {string|null|undefined} remoteServer - For ${REMOTE_HOST}
 * @param {{ skipVdirMergeWhenPathIsBareHealth?: boolean }} [opts] - When `skipVdirMergeWhenPathIsBareHealth`, path `/health` is left as-is (in-container probe; Traefik strips vdir for miso/dataplane). Omit for deployment manifests (public probe needs full vdir).
 * @returns {string} Absolute path starting with /
 */
 ...
 */
function buildTraefikConfig(config, devId, scopeOpts = null, remoteServer = null) {
...
function buildServiceConfig(appName, config, port, devId, runExtras = {}) {
  ...
  return {
    ...
    healthCheck: buildHealthCheckConfig(config, devId, useTraefikScope ? scopeOpts : null, remoteServer),
    traefik: buildTraefikConfig(config, devId, useTraefikScope ? scopeOpts : null, remoteServer),
