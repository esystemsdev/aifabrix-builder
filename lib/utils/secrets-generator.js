...
const { getInfraParameterCatalog, generateValueFromCatalogEntry } = require('../parameters/infra-parameter-catalog');
const {
...
function resolveAppDirForDatabaseKey(key) {
  ...
  if (parsed.appKey === 'miso-controller') return null;
  try {
 ...
 */
function tryGenerateSecretValueFromCatalog(key) {
  let catalogEntry;
  try {
    catalogEntry = getInfraParameterCatalog().findEntryForKey(key);
  } catch (err) {
  ...
  if (g === 'randomBytes32' || g === 'emptyString' || g === 'emptyAllowed' || g === 'literal') {
    return generateValueFromCatalogEntry(key, catalogEntry, crypto);
  }
  return undefined;
}
...
function generateSecretValue(key) {
  const fromCatalog = tryGenerateSecretValueFromCatalog(key);
