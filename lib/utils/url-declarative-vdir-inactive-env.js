...
const { resolveInactiveVdirRewriteEnvKeys } = require('./app-relative-path-env');
 ...
 * @param {ReadonlySet<string>} rewriteKeys
 * @returns {string}
 */
function applyInactiveVdirPublicTokenRewrite(content, replacement, rewriteKeys) {
  ...
  return content.replace(lineRe, (match, indent, key) => {
...
function rewriteInactiveDeclarativeVdirPublicContent(content, variablesPath, userCfg) {
  ...
  const appKey = doc && doc.app && doc.app.key;
  const rewriteKeys = resolveInactiveVdirRewriteEnvKeys(appKey);
  return applyInactiveVdirPublicTokenRewrite(content, INACTIVE_VDIR_PUBLIC_ENV_FALLBACK, rewriteKeys);
}

module.exports = {
  URL_DECLARATIVE_VDIR_PUBLIC_TOKEN,
  INACTIVE_VDIR_PUBLIC_ENV_FALLBACK,
  loadApplicationYamlDoc,
