/**
 * @fileoverview Single-capability CLI contract when positional [capabilityKey] is set.
...
function analyzeCapabilityScope(envelope, requestedCapabilityKey) {
  ...
  return {
    violated: true,
    count: caps.length,
    message: `Capabilities scope: with capabilityKey "${key}", expected a single row in capabilities[]; server returned ${caps.length} (${preview}${suffix}).`
  };
}
