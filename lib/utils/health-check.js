...
async function computeHealthCheckUrl(appName, healthCheckPort, appConfig, _opts = {}) {
  ...
  function computeLocalhostHealthPath() {
    // For localhost probing, align with docker-compose healthcheck path rules:
    // - Keycloak-style: prepend the frontDoorRouting base (e.g. /auth) to /health/ready
    // - Miso/dataplane-style: keep bare "/health" un-prefixed (ingress may mount at /miso, but service listens at /health)
    try {
      ...
      const shouldMount =
        Boolean(fd && fd.enabled === true && pattern) && rawHealthPath !== '/health';
      ...
      // joinUrlPath also normalizes duplicate slashes; it returns an absolute path when left is "/".
  ...
  // Default for `aifabrix run`: always probe the locally published port.
  //
  // Rationale: local container readiness must not depend on the edge proxy (nginx) or Traefik being up/healthy.
  // Traefik may intentionally filter unhealthy containers or be offline, while the container is reachable via localhost.
  //
  // Note: callers can opt into probing via Traefik/front door by passing runOptions.probeViaTraefik=true.
  async function maybeGetTraefikUrl() {
    const runOptions = (_opts && typeof _opts === 'object') ? _opts.runOptions : null;
    if (!runOptions || runOptions.probeViaTraefik !== true) return '';
    try {
...
async function isHostnameResolvable(hostname, debug) {
  ...
  try {
    ...
    if (err && err.code === 'ENOTFOUND') {
      logger.log(
        chalk.yellow(
          `⚠ DNS name not resolvable on this host: ${hostname}. Falling back to localhost health check.`
        )
      );
    } else if (debug) {
...
async function computePreferredHealthCheckUrls(appName, healthCheckPort, config, runOptions, debug) {
  const localhostUrl = await computeHealthCheckUrl(appName, healthCheckPort, config, { runOptions: {} });
  ...
  if (traefikUrl) {
    try {
      ...
      const ok = await isHostnameResolvable(hn, debug);
      if (!ok) traefikUrl = '';
    } catch {
      traefikUrl = '';
    }
...
async function waitForHealthCheck(appName, timeout = 90, port = null, config = null, debug = false, runOptions = {}) {
  ...
  const urlsToTry = await computePreferredHealthCheckUrls(
    appName,
    healthCheckPort,
    config,
    runOptions,
  ...
  for (let attempts = 0; attempts < maxAttempts; attempts++) {
    const passed = await performHealthCheckAttemptForUrls(urlsToTry, attempts, maxAttempts, debug);
    if (passed) {
      return;
    }

    if (attempts < maxAttempts - 1) {
      const shown = urlsToTry[0] || '';
      logger.log(chalk.yellow(`Waiting for health check... (${attempts + 1}/${maxAttempts}) ${shown}`));
