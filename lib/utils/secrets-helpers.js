...
/**
 * Get secret value by path. Supports flat key (hubspot/clientId) and nested object (hubspot.clientId).
 * @param {Object} secrets - Secrets object (may be nested)
 * @param {string} pathStr - Path after kv:// (e.g. 'hubspot/clientId' or 'hubspot')
...
function getValueByPath(secrets, pathStr) {
  ...
  if (pathStr.includes('/')) {
    ...
    for (const part of parts) {
      value = value && typeof value === 'object' && part in value ? value[part] : undefined;
