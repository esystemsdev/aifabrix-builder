...
/**
 * Get value by flat key with slashes replaced by hyphens (e.g. hubspot-clientid).
 * @param {Object} secrets - Secrets object
 * @param {string} pathStr - Path with slashes
 * @returns {*} Value or undefined
 */
function getValueByHyphenKey(secrets, pathStr) {
  const hyphenKey = findKeyCaseInsensitive(secrets, pathStr.replace(/\//g, '-'));
  return hyphenKey !== undefined ? secrets[hyphenKey] : undefined;
}
...
function getValueByPath(secrets, pathStr) {
  ...
  const value = getValueByNestedPath(secrets, pathStr.split('/'));
  if (value !== undefined) return value;
  return getValueByHyphenKey(secrets, pathStr);
}
