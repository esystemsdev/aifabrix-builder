...
function findKeyCaseInsensitive(obj, part) {
  if (!obj || typeof obj !== 'object' || part == null) return undefined;
 ...
 * flat key with hyphens (hubspot-clientid), and case-insensitive matching (clientid matches clientId).
...
function getValueByPath(secrets, pathStr) {
  ...
  let value = secrets[pathStr];
  if (value !== undefined) {
    return value;
  }
  // Case-insensitive flat key (e.g. hubspot/clientid matches hubspot/clientId in YAML)
  const flatKey = findKeyCaseInsensitive(secrets, pathStr);
  if (flatKey !== undefined) {
    return secrets[flatKey];
  }
  if (pathStr.includes('/')) {
    ...
    // Fallback: flat key with slashes replaced by hyphens (e.g. hubspot-clientid for kv://hubspot/clientid)
    const hyphenKey = findKeyCaseInsensitive(secrets, pathStr.replace(/\//g, '-'));
    value = hyphenKey !== undefined ? secrets[hyphenKey] : undefined;
  }
  return value;
}
