 ...
 * - `aifabrix-work` / `AIFABRIX_WORK` when that path contains `integration/` or `builder/`.
 * - Else project root when cwd is inside that root (e.g. run from `integration/<app>/` in the repo).
 * - Else {@link getStableIntegrationBuilderFallbackBaseDir} (config + `aifabrix-home` rules — never raw cwd).
 *
