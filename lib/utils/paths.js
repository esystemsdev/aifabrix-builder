...
function getProjectRoot() {
  ...
  // In test environments, trust it completely - test setup already verified it exists
  const isTestEnv = process.env.NODE_ENV === 'test' || process.env.JEST_WORKER_ID !== undefined;

  if (typeof global !== 'undefined' && global.PROJECT_ROOT) {
    const globalRoot = global.PROJECT_ROOT;

    // In test environments, verify global.PROJECT_ROOT has package.json before using it
    // This is especially important in CI where project is copied to temp directory
    // If global.PROJECT_ROOT doesn't have package.json, it might point to wrong location
    if (isTestEnv) {
      // Verify global.PROJECT_ROOT has package.json (important in CI where project is copied to temp dir)
      if (hasPackageJson(globalRoot)) {
        // Also verify templates directory exists to ensure we're in the right project
        // This catches cases where global.PROJECT_ROOT points to original project but we're in temp copy
        const templatesDir = path.join(globalRoot, 'templates');
        if (fs.existsSync(templatesDir)) {
          // Always use global.PROJECT_ROOT if set in tests (don't cache, as it might change between tests)
          // Clear cache to ensure we don't use stale cached value
          cachedProjectRoot = null;
          return globalRoot;
        }
      }
      // If global.PROJECT_ROOT doesn't have package.json or templates, clear it and continue with other strategies
      // This handles cases where global.PROJECT_ROOT points to wrong location (e.g., original project in CI)
      // Clear the invalid global.PROJECT_ROOT so we don't keep trying it
      if (typeof global !== 'undefined') {
        delete global.PROJECT_ROOT;
      }
    }

    // In non-test environments, verify it actually has package.json (defensive check)
    if (hasPackageJson(globalRoot)) {
    ...
    // If global.PROJECT_ROOT is set but invalid, clear it and continue with other strategies
    // This prevents infinite loops if global.PROJECT_ROOT is incorrectly set
  }
  ...
  // Start from __dirname (lib/utils/) which is most reliable
  ...
  // Fallback strategy - resolve from __dirname
  const fallbackRoot = getFallbackProjectRoot();
  // Verify fallback has package.json
