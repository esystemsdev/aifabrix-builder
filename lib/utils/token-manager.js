...
} = require('./token-manager-client-credentials');
...
async function getOrRefreshDeviceToken(controllerUrl) {
  ...
  if (!tokenInfo.refreshToken) {
    const { isSetupPlatformFlow } = require('../commands/setup-run-context');
    if (!isSetupPlatformFlow()) {
      logger.warn(
        'Access token expired and no refresh token available. Please login again using: aifabrix login'
      );
    }
    return null;
  }
 ...
 */
async function getDeploymentAuth(controllerUrl, environment, appName, options = {}) {
  const { getDeploymentAuthMode, shouldUseDeviceTokenForDeployment } = require('./deployment-auth-mode');
  validateDeploymentAuthParams(controllerUrl, environment, appName);
  const mode = getDeploymentAuthMode(options);

  if (shouldUseDeviceTokenForDeployment(mode)) {
    ...
    if (mode === 'device') {
      throw new Error(
        'Device token required (AIFABRIX_DEPLOYMENT_AUTH=device). Run: aifabrix login'
      );
    }
  }

  const { credentialApp, credentials } = await resolveCredentialAppForDeployment(appName);

  const clientTokenAuth = await tryClientTokenAuth(environment, credentialApp, controllerUrl);
  if (clientTokenAuth) {
  ...
  if (credentials?.clientId && credentials?.clientSecret) {
    try {
      const refreshed = await refreshClientToken(environment, credentialApp, controllerUrl);
      if (refreshed?.token) {
        return {
          type: 'client-token',
          token: refreshed.token,
  ...
  throw new Error(
    'No authentication method available. For CI: set AIFABRIX_DEPLOYMENT_AUTH=client-credentials and ' +
      `MISO_CLIENTID/MISO_CLIENTSECRET in .env or '${credentialApp}-client-idKeyVault' in secrets.local.yaml. ` +
      'For interactive use: aifabrix login'
  );
}
...
/**
 * Ensures auth config has a token Dataplane pipeline endpoints accept (Bearer or x-client-token).
 * Dataplane pipeline rejects raw client id/secret; use POST /api/v1/auth/token first.
 * When {@link process.env.AIFABRIX_DEPLOYMENT_AUTH} is `client-credentials`, keep application
 * `x-client-token` auth (do not substitute API_KEY).
 ...
 */
/**
 * Lab bypass: records/search accepts Bearer API_KEY but rejects x-client-token on some stacks.
 * @param {Object} authConfig
 * @returns {Object}
 */
function applyLabDataplaneApiKeyBearer(authConfig) {
  const labKey = process.env.AIFABRIX_DATAPLANE_API_KEY || '';
  if (!labKey) {
    return authConfig;
  }
  authConfig.type = 'bearer';
  authConfig.token = String(labKey);
  return authConfig;
}

function requireBearerForDataplanePipeline(authConfig) {
  if (authConfig.type === 'client-token' && authConfig.token) {
    return;
  }

  const preferAppClientToken =
    String(process.env.AIFABRIX_DEPLOYMENT_AUTH || '').toLowerCase() === 'client-credentials';

  if (!preferAppClientToken && process.env.API_KEY) {
    const apiKey = String(process.env.API_KEY);
    if (authConfig.type === 'bearer' && authConfig.token !== apiKey) {
      authConfig.token = apiKey;
      return;
    }
    if (authConfig.type === 'client-token') {
      authConfig.type = 'bearer';
      authConfig.token = apiKey;
      return;
    }
  ...
  if (!preferAppClientToken && process.env.API_KEY) {
    authConfig.type = 'bearer';
    authConfig.token = String(process.env.API_KEY);
    return;
  }

  if (authConfig.clientId || authConfig.clientSecret) {
    throw new Error(
      'Dataplane pipeline endpoints require OAuth2 (Bearer token) or x-client-token. ' +
        'Client id/secret are not accepted. Run "aifabrix login" to authenticate, then retry.'
    );
  }
}
