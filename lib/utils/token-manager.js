...
function requireBearerForDataplanePipeline(authConfig) {
  // If an API_KEY is available, prefer it for dataplane pipeline calls even when a device Bearer exists.
  // Dataplane pipeline accepts Authorization: Bearer <API_KEY> as a bypass token; some dev stacks reject controller device tokens.
  if (process.env.API_KEY && authConfig.type === 'bearer') {
  ...
  // Dataplane supports API_KEY bypass via Authorization: Bearer <API_KEY>.
  // This is useful in local/dev environments where device login is unavailable in CI/scripts.
  //
  // NOTE: Dataplane pipeline does NOT accept application client tokens (x-client-token).
  // If we have a client-token authConfig, prefer API_KEY if available; otherwise require device login.
  if (authConfig.type === 'client-token') {
    ...
    throw new Error(
      'Dataplane pipeline endpoints require OAuth2 (Bearer token) or API_KEY. Application client tokens are not accepted. Run "aifabrix login" to authenticate, then retry.'
    );
  }
  if (authConfig.token) {
    return;
