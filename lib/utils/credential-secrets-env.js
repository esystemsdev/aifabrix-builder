 ...
 */
/**
 * For bearerToken auth, credential template resolves security.token only.
 * Legacy .env keys KV_*_APIKEY with a plain token wrongly map to kv://.../apiKey.
 * Prefer token path; drop duplicate apiKey when both would be pushed.
 *
 * @param {Map<string, string>} itemsByKey
 * @param {string} [systemKey]
 * @param {Object} [authConfig]
 */
function reconcileBearerTokenSecretPaths(itemsByKey, systemKey, authConfig) {
  const method = (
    authConfig?.method ||
    authConfig?.type ||
    ''
    ...
    .toLowerCase()
    .replace(/_/g, '');
  if (method !== 'bearertoken' || !systemKey) return;
  const tokenPath = `kv://${systemKey}/token`;
  const apiKeyPath = `kv://${systemKey}/apiKey`;
  if (!itemsByKey.has(apiKeyPath)) return;
  if (!itemsByKey.has(tokenPath)) {
    itemsByKey.set(tokenPath, itemsByKey.get(apiKeyPath));
  }
  itemsByKey.delete(apiKeyPath);
}

/**
 * Optional dev/lab dataplane API key (Bearer bypass) for credential secret push only.
 * Does not alter pipeline publish auth when AIFABRIX_DEPLOYMENT_AUTH=client-credentials.
 * @returns {string|null}
 */
function resolveLabDataplaneApiKey() {
  const candidates = [
    process.env.AIFABRIX_DATAPLANE_API_KEY,
    process.env.DATAPLANE_API_KEY,
    process.env.API_KEY
  ];
 ...
 */
function apiKeyAuthConfig(token) {
  return { type: 'bearer', token };
}

const PERMISSION_DENIED_WARNING =
  'Could not push credential secrets (permission denied or unauthenticated). Ensure dataplane role has credential:create if you use KV_* in .env, or set AIFABRIX_DATAPLANE_API_KEY for local lab bypass.';

/**
 * @param {Object} res
 * @returns {boolean}
 */
function isStoreResponseFailed(res) {
  return Boolean(res && (res.success === false || res.data?.success === false));
}
 ...
 */
function storeResponseStatus(res) {
  return res?.status ?? res?.statusCode;
}

/**
 * @param {Object} res
 * @returns {{ pushed: number, warning: string }}
 */
function storeErrorWarning(res) {
  const errMsg =
    res.formattedError ||
    res.data?.formattedError ||
    res.error ||
    res.data?.error ||
    'Failed to push credential secrets to dataplane.';
 ...
 * @returns {Promise<{ pushed: number }|null>}
 */
async function tryLabApiKeyRetry(dataplaneUrl, authConfig, items) {
  const labKey = resolveLabDataplaneApiKey();
  if (!labKey || authConfig?.token === labKey) {
    return null;
  }
  const retry = await storeCredentialSecrets(
    dataplaneUrl,
    apiKeyAuthConfig(labKey),
    items
  );
  if (isStoreResponseFailed(retry)) {
    return null;
  }
  return { pushed: storedCountFromResponse(retry, items.length) };
}
 ...
 * @returns {Promise<{ pushed: number, warning?: string }>}
 */
async function handleFailedStoreResponse(dataplaneUrl, authConfig, items, res) {
  const status = storeResponseStatus(res);
  if (status === 403 || status === 401) {
    const retryResult = await tryLabApiKeyRetry(dataplaneUrl, authConfig, items);
    if (retryResult) {
      return retryResult;
    }
    return { pushed: 0, warning: PERMISSION_DENIED_WARNING };
  }
  return storeErrorWarning(res);
}
...
async function sendCredentialSecrets(dataplaneUrl, authConfig, items) {
  try {
    ...
    if (isStoreResponseFailed(res)) {
      return handleFailedStoreResponse(dataplaneUrl, authConfig, items, res);
    }
    return { pushed: storedCountFromResponse(res, items.length) };
  } catch (err) {
    return {
      pushed: 0,
      warning: err.message || 'Failed to push credential secrets to dataplane.'
    };
  }
}
...
async function pushCredentialSecrets(dataplaneUrl, authConfig, options = {}) {
  ...
  reconcileBearerTokenSecretPaths(itemsByKey, appName, authConfig);
