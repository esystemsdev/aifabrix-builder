...
function readIssueCertPin() {
  ...
  try {
    const content = require('fs').readFileSync(path.normalize(fp.trim()), 'utf8');
...
async function readEnsureIssueCertContext() {
  ...
  const certDir = getCertDir(configDir, devId);
  if (
    require('fs').existsSync(path.join(certDir, 'cert.pem')) &&
    require('fs').existsSync(path.join(certDir, 'key.pem'))
  ) {
    return null;
  }
