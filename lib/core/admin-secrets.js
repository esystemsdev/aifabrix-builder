...
/**
 * Whether an admin-secrets.env value must be double-quoted for safe round-trip parsing.
 * @param {string} value
 * @returns {boolean}
 */
function needsAdminEnvQuoting(value) {
  const s = String(value ?? '');
  return /[,\s#=]/.test(s) || s !== s.trim();
}

/**
 * Format one KEY=value line for admin-secrets.env (quotes when needed).
 * @param {string} key
 * @param {string} value - Plaintext value (not yet encrypted)
 * @returns {string}
 */
function formatAdminEnvLine(key, value) {
  const safe = String(value ?? '').replace(/\n/g, ' ').trim();
  if (!needsAdminEnvQuoting(safe)) {
    return `${key}=${safe}`;
  }
...
function envObjectToContent(obj) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    lines.push(formatAdminEnvLine(key, value));
  }
  return lines.join('\n');
}
