...
async function resolveAndGetEnvMap(appName, options = {}) {
  ...
  const map = parseEnvContentToMap(content);
  try {
    const adminSecrets = require('./admin-secrets');
    const { applyAdminSecretsPlatformOverlay } = require('./admin-secrets-env-overlay');
    const adminObj = await adminSecrets.readAndDecryptAdminSecrets();
    applyAdminSecretsPlatformOverlay(map, adminObj);
  } catch {
    // admin-secrets.env optional until up-infra/setup
  }
  return map;
}
