...
} = require('./config-setup-installation-profile');
const {
  getSetupPlatformModeFromConfig,
  setSetupPlatformModeInConfig
} = require('./config-setup-platform-mode');
...
} = require('./config-platform-controller');
...
function applyConfigDefaults(config) {
  ...
  if (typeof config.setupPlatformMode === 'undefined') {
    config.setupPlatformMode = 'single';
  }
...
const getSetupInstallationProfile = () => getSetupInstallationProfileFromConfig(getConfig);
/** @param {unknown} profile @returns {Promise<'dev'|'pro'>} */
const setSetupInstallationProfile = (profile) =>
  setSetupInstallationProfileInConfig(getConfig, saveConfig, profile);
 ...
 */
async function getSetupPlatformMode() {
  return getSetupPlatformModeFromConfig(getConfig);
}
 ...
 * @returns {Promise<'single'|'full'>}
 */
async function setSetupPlatformMode(mode) {
  return setSetupPlatformModeInConfig(getConfig, saveConfig, mode);
}
...
async function ensureSecretsEncryptionKey() {
  ...
  const { createSecretsPathFunctions } = require('../utils/config-secrets-path');
  const { getSecretsPath } = createSecretsPathFunctions(getConfig, saveConfig);
...
const exportsObj = {
  ...
  getAdminEmail,
  setAdminEmail,
  getSetupInstallationProfile,
  setSetupInstallationProfile,
  getSetupPlatformMode,
  setSetupPlatformMode,
  getTraefikEnabled,
  ...
  setControllerUrl,
  getControllerUrl,
  getPlatformControllerUrl: () => getPlatformControllerUrlFromConfig(getConfig),
  setPlatformControllerUrl: (url) => setPlatformControllerUrlInConfig(getConfig, saveConfig, normalizeControllerUrl, url),
...
// Developer ID: 0 = default infra, > 0 = developer-specific (getter after getConfig loads cache)
