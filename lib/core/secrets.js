...
async function loadSecrets(secretsPath, _appName) {
  ...
  // Fallback: merge builder/secrets.local.yaml from project root (existing keys win)
  try {
    const projectRoot = pathsUtil.getProjectRoot();
    if (projectRoot) {
      const builderSecretsPath = path.join(projectRoot, 'builder', 'secrets.local.yaml');
      if (fs.existsSync(builderSecretsPath)) {
        const builderSecrets = mergeUserWithConfigFile(mergedSecrets || {}, builderSecretsPath);
        if (builderSecrets) mergedSecrets = builderSecrets;
      }
    }
  } catch {
