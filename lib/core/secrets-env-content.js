...
function resolveEnvContentToWrite(resolved, pathToPreserve, options = {}) {
  ...
  let merged = mergeEnvMapIntoContent(existingContent, resolvedMap, { appendMissingFromNewMap: true });
  if (options.pruneOrphanKv === true) {
    const { content: pruned } = commentOrphanKvEnvLines(merged, new Set(Object.keys(resolvedMap)));
    merged = pruned;
  }
  return merged;
}
...
async function writeResolvedEnv({ appName, envPath, resolved, variablesPath, secretsPath, opts }) {
  ...
  const toWrite = resolveEnvContentToWrite(resolved, pathToPreserve, {
    ...
    pruneOrphanKv: opts.envOnly === true && opts.freshEnv !== true
  });
