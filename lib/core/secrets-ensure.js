...
function isSecretKeyAllowedEmpty(key) {
  try {
    return getInfraParameterCatalog().isKeyAllowedEmpty(key);
  } catch {
    const emptyAllowed = readRelaxedEmptyAllowedKeySet(DEFAULT_CATALOG_PATH);
...
function getInfraSecretKeysForUpInfra() {
  try {
    const catalog = getInfraParameterCatalog();
    ...
    const relaxed = readRelaxedUpInfraEnsureKeyList(DEFAULT_CATALOG_PATH);
...
async function ensureInfraSecrets(options = {}) {
  ...
  return ensureSecretsForKeys(keys, { suggestedValues: suggested });
}
...
Object.defineProperty(module.exports, 'INFRA_SECRET_KEYS', {
  enumerable: true,
  get() {
    return readRelaxedUpInfraEnsureKeyList(DEFAULT_CATALOG_PATH) || [];
  }
});
