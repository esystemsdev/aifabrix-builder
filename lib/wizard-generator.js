...
async function generateWizardFiles(appName, systemConfig, datasourceConfigs, systemKey) {
  try {
    ...
    await generateOrUpdateVariablesYaml(appPath, appName, finalSystemKey, systemFileName, datasourceFileNames, systemConfig);
    ...
    // Generate application-schema.json (uses the files we just wrote)
 ...
 */
async function generateOrUpdateVariablesYaml(appPath, appName, systemKey, systemFileName, datasourceFileNames, systemConfig) {
  try {
