 ...
 */
async function finalizeProtectionCreateWrite({ dsKey, dryRun, force, manifest, logger }) {
  const format = await resolveProtectionOutputFormat();
  const body = protectionManifestToDisplayText(manifest, format);
  if (dryRun) {
    logger.log(metadata('Dry run: no file written'));
    process.stdout.write(body);
    return 0;
  }
  const { outputPath } = await resolveOutputPathOrThrow(dsKey, force);
  writeProtectionManifest(outputPath, manifest, format);
...
async function runProtectionCreate(datasourceKey, opts, logger) {
  ...
  return finalizeProtectionCreateWrite({ dsKey, dryRun, force, manifest, logger });
}
