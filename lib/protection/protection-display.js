...
const { describeProtectionRoot } = require('./paths');
const {
  appendTaskRows,
 ...
 * @returns {string}
 */
function formatProtectionBatchFolderLine(opts = {}) {
  const described = describeProtectionRoot();
  const folderPath = opts.folder ? path.resolve(String(opts.folder)) : described.root;
  const lines = [metadata(`Folder: ${described.label} — ${folderPath}`)];
  if (described.migrationHint) {
    lines.push(formatWarningLine(described.migrationHint));
  }
  return lines;
}
...
function formatProtectionBatchValidateTTY(results, opts = {}) {
  ...
  lines.push(...formatProtectionBatchFolderLine(opts));
  lines.push('');
  let passed = 0;
  let failed = 0;
...
function formatProtectionShowTTY(payload, opts = {}) {
  const lines = [];
  appendProtectionShowHeader(lines, payload, opts);
  ...
  appendProtectionShowContextFooter(lines, payload);
  return lines.join('\n');
}
