/**
 * @fileoverview Resolve protection manifests by datasource key under `{work}/.protection/`.
...
function resolveProtectionManifest(datasourceKey, explicitPath) {
  ...
  const preferred = path.join(root, `${key}.yaml`);
  ...
  if (matches.length === 0) {
    throw new Error(
      `No protection manifest for datasource "${key}" under ${root}. Expected ${key}.yaml`
    );
  }
  if (matches.length > 1) {
    throw new Error(
