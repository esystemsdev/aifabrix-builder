...
function getRepoProtectionRoot() {
  ...
  const materializationParent = path.resolve(getAppsMaterializationParent());
  const nearest = findNearestIntegrationRootFromCwd();
  if (nearest) {
    const resolvedNearest = path.resolve(nearest);
    if (
      resolvedNearest === materializationParent ||
      resolvedNearest.startsWith(`${materializationParent}${path.sep}`)
    ) {
      return path.join(resolvedNearest, '.protection');
    }
  }
  return path.join(materializationParent, 'integration', '.protection');
}
