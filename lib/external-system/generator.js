 ...
 * @param {string} authType - Auth method (oauth2, aad, apikey, bearerToken, basic, queryParam, oidc, hmac, none)
...
function buildAuthenticationFromMethod(systemKey, authType) {
  ...
  const authMap = {
    ...
    bearerToken: {
      variables: { baseUrl: base, headerName: 'Authorization', prefix: 'Bearer' },
      security: { token: kvPath('token') }
    },
    basic: {
