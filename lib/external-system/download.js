...
async function downloadExternalSystem(systemKey, options = {}) {
  ...
  try {
    ...
    const { authConfig, dataplaneUrl } = await setupAuthenticationAndDataplane(
      systemKey,
      options,
      config
    );
    ...
    const finalPath = getIntegrationPath(systemKey);
    await processDownloadedSystem(
      systemKey,
      manifest,
      await resolveDownloadSplitOptions(finalPath, options)
    );
    const { downloadGovernanceScenarioPacksFromManifest } = require('../lifecycle/scenario-upload');
    await downloadGovernanceScenarioPacksFromManifest(
      systemKey,
      manifest,
      dataplaneUrl,
      authConfig
