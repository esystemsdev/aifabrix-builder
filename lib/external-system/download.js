 ...
 */
/**
 * Generates system file with dataSources keys in sync with application config file names.
 * @async
 * @function generateSystemFile
 * @param {string} tempDir - Temporary directory
 * @param {string} systemKey - System key
 * @param {Object} application - Application object from API
 * @param {Array} dataSources - Array of datasource objects (used to set application.dataSources keys)
 * @returns {Promise<string>} System file path
 */
async function generateSystemFile(tempDir, systemKey, application, dataSources) {
  const systemFileName = `${systemKey}-system.yaml`;
  const systemFilePath = path.join(tempDir, systemFileName);
  const dataSourcesKeys = Array.isArray(dataSources) ? dataSources.map(ds => ds.key) : [];
  const applicationWithDataSourcesKeys = { ...application, dataSources: dataSourcesKeys };
  writeConfigFile(systemFilePath, applicationWithDataSourcesKeys);
...
async function generateFilesInTempDir(tempDir, systemKey, application, dataSources) {
  const systemFilePath = await generateSystemFile(tempDir, systemKey, application, dataSources);
...
async function processDownloadedSystem(systemKey, application, dataSources, tempDir) {
  ...
  // Downloads always create integration/<systemKey>/ — no pre-existing app required
