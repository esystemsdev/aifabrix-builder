...
/**
 * Resolves split options for download: merge env.template if it exists, prompt for README replace if it exists.
 * @param {string} finalPath - Integration directory path
 * @returns {Promise<{ mergeEnvTemplate: boolean, overwriteReadme: boolean }>}
 */
async function resolveDownloadSplitOptions(finalPath) {
...
async function processDownloadedSystem(systemKey, manifest, splitOptions = {}) {
  ...
  await generator.splitDeployJson(deployJsonPath, finalPath, splitOptions);
...
async function downloadExternalSystem(systemKey, options = {}) {
  ...
  try {
    ...
    // Resolve split options: merge existing env.template, prompt for README replace
    const finalPath = getIntegrationPath(systemKey);
    const splitOptions = await resolveDownloadSplitOptions(finalPath);
