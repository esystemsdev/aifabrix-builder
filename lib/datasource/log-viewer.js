...
const { statusGlyph } = require('../utils/datasource-test-run-display');
const { trustDecisionToRowStatus } = require('./agent-trust-map');
 ...
 * @param {string} [fileName] - Log file name for header
 */
function trustLogGlyph(trustDecision) {
  return statusGlyph(trustDecisionToRowStatus(trustDecision));
}

function logTrustWarningsSection(run) {
  const warnings = Array.isArray(run.highLevelWarnings) ? run.highLevelWarnings : [];
  if (!warnings.length) return;
  logger.log(sectionTitle('Warnings:'));
  for (const w of warnings) {
    logger.log(`  ${chalk.yellow('•')} ${chalk.white(String(w))}`);
  }
}

function logTrustFindingsSection(run) {
  const findings = Array.isArray(run.findings) ? run.findings : [];
  if (!findings.length) return;
  logger.log(sectionTitle('Findings:'));
  const cap = 15;
  for (let i = 0; i < Math.min(cap, findings.length); i += 1) {
    const f = findings[i];
    const code = f?.code || f?.stableCode || 'finding';
    const msg = f?.message ? String(f.message) : '';
    logger.log(`  ${chalk.gray(`• ${code}`)} ${msg}`.trim());
  }
  if (findings.length > cap) {
    logger.log(chalk.gray(`  … and ${findings.length - cap} more (open JSON file for full list)`));
  }
}

/**
 * Format agent trust log (`datasource verify-trust --debug` → `verify-trust-*.json`).
 * @param {Object} data - Parsed JSON { request, response?, error? }
 * @param {string} [fileName] - Log file name for header
 */
function formatTrustLog(data, fileName) {
  logger.log('');
  logger.log(sectionTitle('Agent trust log'));
  if (fileName) logger.log(chalk.gray(fileName));
  const req = data.request || {};
  logger.log('');
  logger.log(sectionTitle('Request:'));
  logger.log(headerKeyValue('datasourceKey:', String(req.datasourceKey ?? '—')));
  if (req.revalidate !== undefined) {
    logger.log(chalk.gray(`  revalidate: ${req.revalidate}`));
  }
  if (data.error) {
    logger.log('');
    logger.log(formatBlockingError(`Error: ${data.error}`));
  ...
  const run = data.response || {};
  logger.log('');
  logger.log(sectionTitle('Response:'));
  logger.log(headerKeyValue('System:', String(run.systemKey ?? '—')));
  const confPct = Math.round((Number(run.confidence) || 0) * 100);
  logger.log(
    headerKeyValue(
      'Trust:',
      `${trustLogGlyph(run.trustDecision)} ${run.trustDecision ?? '—'} · ${confPct}% · ${run.validationStatus ?? '—'}`
    )
  );
  if (run.validatedAt) {
    logger.log(chalk.gray(`  validatedAt: ${run.validatedAt}`));
  }
  if (run.inputHash) {
    logger.log(chalk.gray(`  inputHash: ${truncate(String(run.inputHash), 40)}`));
  }
  if (run.cacheHit !== undefined && run.cacheHit !== null) {
    logger.log(chalk.gray(`  cacheHit: ${run.cacheHit}`));
  }
  if (run.summary) {
    logger.log('');
  ...
  logTrustWarningsSection(run);
  logTrustFindingsSection(run);
}
 ...
 * @param {'test'|'test-e2e'|'test-integration'|'verify-trust'} logType - Log type
...
function formatLogContent(parsed, logType, fileName) {
  ...
  if (logType === 'verify-trust') {
    formatTrustLog(parsed, fileName);
    return;
  }
  if (logType === 'test-e2e') {
 ...
 * @param {'test'|'test-e2e'|'test-integration'|'verify-trust'} options.logType - Log type
...
async function runLogViewer(datasourceKey, options) {
  ...
  if (file && typeof file === 'string' && file.trim()) {
    ...
    } else if (logType === 'verify-trust') {
      logPath = await getLatestLogPath(logsDir, 'verify-trust-');
      if (!logPath) {
        throw new Error(
          `No verify-trust log found in ${logsDir}. Run: aifabrix datasource verify-trust <key> --debug`
        );
      }
    } else {
