...
/**
 * Format agent trust log (`datasource test-trust --debug` → `test-trust-*.json`).
 * @param {Object} data - Parsed JSON { request, response?, error? }
 * @param {string} [fileName] - Log file name for header
 */
 ...
 * @param {'test'|'test-e2e'|'test-integration'|'test-trust'} logType - Log type
 ...
 * @param {'test'|'test-e2e'|'test-integration'|'test-trust'} options.logType - Log type
...
async function runLogViewer(datasourceKey, options) {
  ...
  if (file && typeof file === 'string' && file.trim()) {
    ...
    } else if (logType === 'test-trust') {
      logPath = await getLatestLogPath(logsDir, 'test-trust-');
      if (!logPath) {
        throw new Error(
          `No test-trust log found in ${logsDir}. Run: aifabrix datasource test-trust <key> --debug`
        );
      }
    } else {
