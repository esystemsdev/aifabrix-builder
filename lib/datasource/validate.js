...
function pickIntegrationAppForDatasourceKey(datasourceKey, appNames) {
  return pickIntegrationAppForDatasourceIdentifier(datasourceKey, appNames);
}
...
function collectJsonPathsMatchingDatasourceKey(dir, scanOrder, datasourceKey) {
  ...
  for (const name of scanOrder) {
    ...
    if (datasourceIdentifierMatchesFile(datasourceKey, name, parsed)) {
      hits.push(full);
    }
  }
  return hits;
 ...
 * @throws {Error}
 */
function resolvePathFromIntegrationFolderScan(appName, datasourceKey) {
  ...
  if (hits.length === 0) {
    throw new Error(
      `No datasource JSON matching "${datasourceKey}" under integration/${appName}/. ` +
        `Checked: ${scanOrder.length ? scanOrder.join(', ') : '(no JSON candidates)'}`
    );
  }
  if (hits.length > 1) {
    throw new Error(`Ambiguous: multiple files match "${datasourceKey}": ${hits.join(', ')}`);
  }
  return hits[0];
}
...
function resolvePathFromIntegrationDatasourceKey(datasourceKey) {
  ...
  if (!appName) {
    throw new Error(
      `No integration/<app>/ folder matches datasource identifier "${datasourceKey}". ` +
        'Pass a path to the JSON file, or use a datasource key or filename whose prefix matches an app under integration/ ' +
        '(e.g. test-e2e-hubspot-companies or test-e2e-hubspot-datasource-companies for app test-e2e-hubspot).'
    );
  }
  const manifestHits = collectDatasourceFileRefsMatchingIdentifier(appName, datasourceKey);
  if (manifestHits.length === 1) {
    const dir = pathsUtil.getIntegrationPath(appName);
    return path.join(dir, manifestHits[0]);
  }
  return resolvePathFromIntegrationFolderScan(appName, datasourceKey);
}
