 ...
 * @returns {Promise<import('../api/types/audit.types').AuditEvidenceMatrixRow[]>}
 */
async function buildPerExecutionSubRows(ctx, execIds) {
  const { dataplaneUrl, authConfig } = ctx;
  const base = { dataplaneUrl, authConfig, executionId: execIds[0] };
  return Promise.all([
 ...
 * @returns {Promise<Array>}
 */
async function resolveRow8PoolForMatrix(ctx, executions) {
  const { dataplaneUrl, authConfig } = ctx;
  let corrQueryRows = [];
  ...
  const row8Pool = corrQueryRows.length ? corrQueryRows : executions;
  ...
  return byOp.length ? byOp : row8Pool;
}
 ...
 */
async function collectTraceIdsForRow9(ctx, execIds) {
  const { dataplaneUrl, authConfig } = ctx;
  const traceIds = new Set();
  const sample = execIds.slice(0, Math.min(execIds.length, 12));
...
async function buildPerExecutionAndTailRows(ctx, executions, executionIds) {
  ...
  const rows = await buildPerExecutionSubRows(ctx, execIds);
  const row8Pool = await resolveRow8PoolForMatrix(ctx, executions);
  ...
  rows.push(checkRow9Traces(await collectTraceIdsForRow9(ctx, execIds), execIds));
  return { rows, execIds };
}
