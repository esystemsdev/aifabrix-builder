 ...
 */
async function resolveValidationAuth(appKey, options) {
  if (options.authConfig && (options.dataplaneUrl || options.dataplane)) {
    return {
 ...
 */
async function prepareValidationRunContext(datasourceKey, options) {
  ...
  const { authConfig, dataplaneUrl } = await resolveValidationAuth(appKey, options);
  await maybeSyncDatasourceToDataplane({ options, authConfig, systemKey });
  return { runType, systemKey, datasource, authConfig, dataplaneUrl };
}
...
async function runUnifiedDatasourceValidation(datasourceKey, options) {
  ...
  const ctx = await prepareValidationRunContext(datasourceKey, options);
  ...
  const result = await postValidationRunAndOptionalPoll({
    ...
    pollSubject: datasourceKey,
    onPollProgress: options.onPollProgress
  });

  if (result && result.envelope) {
    annotateMissingAutoRbacPermissions(result.envelope, datasourceKey, ctx.datasource);
  }
  return result;
}
