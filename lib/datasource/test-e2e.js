...
const { formatProgress, formatSuccessLine, infoLine } = require('../utils/cli-test-layout-chalk');
...
function buildUnifiedE2eRunOptions(options, timeoutMs, pk) {
  return {
    ...
    dataplaneUrl: options.dataplaneUrl || options.dataplane,
...
/**
 * Compact per-datasource progress (default test-e2e / system rollup when not --verbose).
 * @param {string} datasourceKey
 * @param {boolean} verbose
 */
function logE2eDatasourceProgress(datasourceKey, verbose) {
  if (verbose) return;
  logger.log(formatProgress(`Running E2E validation for ${datasourceKey}…`));
}
 ...
 * @param {boolean} verbose
 */
function logE2eDatasourceValidated(datasourceKey, verbose) {
  if (verbose) return;
  logger.log(formatSuccessLine(`${datasourceKey} validated`));
}
...
async function runDatasourceTestE2E(datasourceKey, options = {}) {
  ...
  logE2eDatasourceProgress(datasourceKey, options.verbose);
  ...
  logE2eDatasourceValidated(datasourceKey, options.verbose);
