...
/**
 * Default integration/.data output path (file need not exist).
 * @param {Object} opts
 * @param {string} opts.systemKey
 * @param {string} opts.entitySuffix
 * @param {'json'|'ndjson'} [opts.format]
 * @returns {{ filePath: string, format: 'json'|'ndjson' }}
 */
function resolveDefaultDataOutputPath(opts) {
  ...
  return { filePath: dataFilePath(dir, basename, format), format };
}
