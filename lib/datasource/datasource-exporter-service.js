 ...
 */
async function executeDatasourceExportWork(ctx, resolved) {
  const searchBody = {
  ...
  if (!response || response.success === false) {
    const msg = response?.formattedError || response?.error || 'Records search request failed';
    throw new Error(msg);
  }
  ...
  return {
    ...
    excludedAbac: Number(excluded.abac) || 0,
    excludedFilter: Number(excluded.filter) || 0,
...
async function runDatasourceExport(datasourceKey, options = {}) {
  ...
  const payload = await runWithDatasourceLoadExportSpinner(
    () => executeDatasourceExportWork(ctx, resolved),
    {
      operation: 'export',
      datasourceKey: ctx.datasourceKey,
