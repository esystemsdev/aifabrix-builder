...
%% Flow
%% =======================
Variables[variables.yaml<br/>App configuration]:::primary --> EnvTemplate[env.template<br/>Environment variables]:::base
EnvTemplate --> Env[.env<br/>Resolved variables]:::base
Variables --> Rbac[rbac.yaml<br/>Roles & permissions]:::base
Variables --> DeployJson[aifabrix-deploy.json<br/>Deployment manifest]:::base
Env --> Container[Docker Container]:::base
DeployJson --> Controller[Miso Controller]:::medium
...
Example: `${DEV_USERNAME}.aifabrix.dev`, `api.example.com`  
...
**frontDoorRouting.certStore**  
Certificate store name for wildcard certificates.  
Example: `wildcard`, `default`  
*Optional - only needed when using a pre-configured certificate store in Traefik*  
*Requires Traefik to be configured with the corresponding certificate store*
...
- TLS configuration (when `tls: true`)
- Certificate store label (when `certStore` is specified)
...
Controller API URL. When set (HTTPS), it is included in generated deployment and pipeline configuration. For CLI commands, the controller comes from `config.controller` (set via `aifabrix login` or `aifabrix auth config --set-controller`). For CI/pipeline, `MISO_CONTROLLER_URL` or equivalent is often used.  
Example: `https://controller.aifabrix.dev`  
*Optional*
...
**Note:** Client credentials are read from `~/.aifabrix/secrets.local.yaml` using pattern `<app-name>-client-idKeyVault` and `<app-name>-client-secretKeyVault`. Tokens are automatically retrieved or refreshed during deployment.

## Controller, Environment, and Dataplane

### Controller URL

The CLI resolves the controller URL in this order:

1. **config.controller** (`~/.aifabrix/config.yaml`) – Set by `aifabrix login` or `aifabrix auth config --set-controller`
2. **Device tokens** (`config.device`) – Controller URLs from existing device tokens
3. **Developer ID–based default** – `http://localhost:${3000 + (developerId * 100)}` (e.g. ID 0 → 3000, 1 → 3100)

### Environment

1. **config.environment** (`~/.aifabrix/config.yaml`) – Set by `aifabrix login` or `aifabrix auth config --set-environment`
2. **Default:** `dev`

### Dataplane URL

The dataplane URL is discovered from the controller when needed. If the controller cannot provide one, the command fails with an error.

### Error messages

Error messages include the controller URL used or attempted:

- **Authentication errors** – which controller URL failed
- **Network errors** – which controller URL was unreachable
- **API errors** – which controller URL returned the error

### Examples

```bash
# 1) Set controller and environment (saved to config.yaml)
...
# 2) Run commands (controller/environment come from config)
...
**Note:** Currently, only the first system in the array is included in the generated `<systemKey>-deploy.json`. Multiple systems are not supported in a single application deployment.  
Example:
```yaml
systems:
  - hubspot-system.json
...
List of external-datasource JSON files belonging to this app. Multiple data sources are supported and all will be included in the generated `<systemKey>-deploy.json`.  
Example:
```yaml
dataSources:
...
- **Only one system per application:** The `systems` array should contain a single system file. Only the first entry is used when generating `<systemKey>-deploy.json`.
- **Multiple data sources supported:** The `dataSources` array can contain multiple datasource files, and all will be included in the generated `<systemKey>-deploy.json`.
...
builder/
  myapp/
    ...
    schemas/                # schemaBasePath
      hubspot-system.json          # External system (only one system supported)
      hubspot-datasource-deal.json     # External datasource
      hubspot-datasource-contact.json  # External datasource
      hubspot-datasource-company.json  # External datasource
```
...
- Merged configuration validates against `<systemKey>-deploy.json`
...
%% Flow
%% =======================
Variables[variables.yaml<br/>externalIntegration block]:::primary --> SchemaPath[schemaBasePath<br/>./schemas]:::medium
SchemaPath --> SystemJson[hubspot-system.json<br/>External system]:::base
SchemaPath --> DatasourceJson1[hubspot-datasource-deal.json<br/>Datasource]:::base
SchemaPath --> DatasourceJson2[salesforce-contact.json<br/>Datasource]:::base

SystemJson --> Pipeline[Pipeline API]:::base
DatasourceJson1 --> Pipeline
DatasourceJson2 --> Pipeline

Pipeline --> Controller[Miso Controller]:::base
Pipeline --> Dataplane[Dataplane<br/>Schema Publishing]:::base
```
...
%% Flow
%% =======================
Template[env.template<br/>kv://secret-name]:::primary --> Resolver[Secret Resolver]:::medium
Secrets[secrets.yaml<br/>secret-name: value]:::base --> Resolver
EnvConfig[env-config.yaml<br/>Template variables]:::base --> Resolver
Resolver --> Env[.env<br/>SECRET=value<br/>Resolved]:::base
```
...
traefik: false                           # Optional: include Traefik in aifabrix up (set by --traefik or --no-traefik; when omitted, this is used)
controller: https://controller.aifabrix.dev  # Root-level: saved during login or via auth config --set-controller
environment: miso  # Root-level: currently selected environment (saved during login or via auth config --set-environment)
...
**developer-id**  
Developer ID for local infrastructure isolation. **Read by `aifabrix up`** when `--developer` is not passed.  
Default: `0` (shared infrastructure)  
Example: `1`, `2`, `5` (developer-specific ports)

**aifabrix-home**  
Base directory for AI Fabrix local files (apps, infra, secrets, logs). **Read by `aifabrix up`** for infra directory and admin-secrets path.  
Default: `~/.aifabrix`
...
**traefik**  
Include Traefik reverse proxy when running `aifabrix up`.  
Default: `false`. Set by `aifabrix up --traefik` (persists `true`) or `aifabrix up --no-traefik` (persists `false`). When neither flag is passed, this value is used.  
Used by: `aifabrix up`.

**controller** (root-level)  
Default controller URL used by all non-auth commands.  
Set by `aifabrix login` or `aifabrix auth config --set-controller`.  
Example: `https://controller.aifabrix.dev`

**environment** (root-level)  
Currently selected environment.  
Set by `aifabrix login` (with `--environment`) or `aifabrix auth config --set-environment`.  
Example: `miso`, `dev`, `tst`, `pro`
...
curl https://controller.aifabrix.dev/api/v1/audit/logs?app=myapp
```
...
The deployment JSON file (`<app-name>-deploy.json` for regular apps, `<systemKey>-deploy.json` for external systems) is generated from component files using `aifabrix json <app>`.
