...
The CLI pushes these secrets to the dataplane secret store automatically before upload/publish. **Skip conditions:** If there is no `.env` file, no `KV_*` keys, or values are empty, the credential push step is skipped; no `POST /api/v1/credential/secret` call is made. Dataplane permission **credential:create** is required for the push; see [Online Commands and Permissions](../commands/permissions.md).
