...
```
Automatically uses registry configured in `builder/<appKey>/application.yaml`:
```yaml
image:
...
The builder CLI only sends the deployment manifest (e.g. `builder/<appKey>/<appKey>-deploy.json` for regular apps) to the controller; the controller determines the deployment target based on the deployment type and executes the deployment accordingly. Polls deployment status by default to track progress. For **external** integrations in `integration/<systemKey>/`, the CLI **regenerates `<systemKey>-deploy.json` on disk before deploying** (same output as `aifabrix json <systemKey>`), then sends it—so you do not need to run `json` separately before deploy.
...
# External system in integration/<systemKey> (resolved first; no app register needed)
...
**App path resolution:** The command always resolves the app by checking **`integration/<systemKey>`** first, then **`builder/<appKey>`**. If neither exists, it errors. There is no option to override this order. External deployments (when the resolved path is in `integration/`) do not require `aifabrix app register`; the controller creates and deploys the application automatically.

**Process:**
1. Validates app name format
2. Resolves app path: `integration/<systemKey>` first, then `builder/<appKey>`; if neither exists, errors. If the resolved app is in `integration/`: external deployment flow (validate → manifest → deploy; no app register).
...
8. Generates deployment manifest (e.g. `builder/<appKey>/<appKey>-deploy.json`)
...
- **"Validation failed"** → Check `builder/<appKey>/<appKey>-deploy.json` for missing required fields
...
Get credentials from Dataplane. Use this to see available credentials when choosing "Use existing" in the wizard (Step 3). When the dataplane provides credential status, the list shows a colored icon: ✓ (verified), ○ (pending), ✗ (failed), ⊘ (expired).
...
**Options:** `--active-only`, `--page-size <n>` (default 50).
