...
- **Authentication method** — When `--auth <method>` is provided, repair sets the integration’s authentication to that method (canonical variables and security) and updates env.template accordingly. **`bearerKey`** is a repair preset for HubSpot-style private apps: persisted method `apikey` with `headerName: Authorization`, `prefix: Bearer`, and optional `testEndpoint`. For any repair run, if `authentication.method` is **`apikey`** (including after `--auth apikey` or `--auth bearerKey`) and **`testEndpoint`** is empty, repair prints a **warning** (credential/E2E may fail). After repair, the CLI lists **changed file paths** and suggested next steps (`validate`, fix warnings).
