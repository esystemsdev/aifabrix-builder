...
### Changed
- **`urls.local.yaml` registry refresh (Plan 141 P3):** `refreshUrlsLocalRegistryFromBuilder` scans **canonical builder roots** in fixed order (`getSystemBuilderRoot` → `getBuilderRoot()` → `projectRoot/builder` → `packages/*` → cwd checkout `builder/` when enabled), de-duplicated by resolved path. Duplicate `app.key` entries: **last directory in that order wins** (no cross-root **mtime** comparison; **`AIFABRIX_BUILDER_DIR`** no longer affects scan ordering). `collectLatestApplicationYamlEntriesPerApp` in `urls-local-registry-scan.js` only compares scan index. `excludeCwdBuilderScan` still omits the cwd checkout `builder/` pass.
- **Shared secret and PII masking (`lib/utils/log-redaction.js`):** **`maskSensitiveData`** (formerly only in **`audit-logger.js`**) plus **`maskEnvLine`** / **`maskUrlEmbeddedCredentials`** for **`KEY=value`** lines and URL **`user:password@`** segments. **`audit-logger.js`** and installation-log records import from this module; **`aifabrix logs <app>`** masked env dump uses the same **`maskEnvLine`** implementation to avoid redaction drift.
- **`aifabrix resolve` + `build.envOutputPath`:** When both `<appPath>/.env` and `build.envOutputPath` are written, the CLI now **copies the same resolved file** (plus `/mnt/data` → local `mount/` next to the output path) instead of regenerating a second `local`-flavored `.env` for the output path. **`aifabrix run`** without **`--reload`** refreshes **`build.envOutputPath`** with **`local`**-oriented values (localhost / dev port offsets) for IDE use while the container keeps **`docker`** env from `.env.run`. **`aifabrix run <app> --reload`** merges `.env.run` into the host file **without appending** keys that are not already present as `KEY=` lines (preserves **`aifabrix resolve`** comments; avoids trailing `DB_*` / registry keys from compose-only env). If the host file is missing, **`--reload`** seeds it from the **`local`** template then merges. **`resolveAndWriteEnvFile`** JSDoc clarifies ephemeral **`app-install` / `app-shell` / `app-test`** use. Docs: `docs/running.md`, `docs/commands/utilities.md`.
...
- **Secret resolution — in-memory only for platform bring-up (Plan 139):** `aifabrix register`, `aifabrix rotate-secret`, `aifabrix build`, and `aifabrix up-platform` / `up-miso` / `up-dataplane` no longer write `<appPath>/.env` or `build.envOutputPath`. `generateEnvFile` now accepts a `noWrite` option that resolves `kv://` references in memory (so missing-secret errors still surface) but skips both writes. A **persistent** `<appPath>/.env` and optional `build.envOutputPath` are created by **`aifabrix resolve <app>`**; **`aifabrix run`** uses ephemeral `.env.run` / `.env.run.admin` for compose and, when **`build.envOutputPath`** is set, refreshes that path (**`local`** without **`--reload`**, merge **`.env.run`** with **`--reload`** per the first bullet above). ISO 27001 alignment: resolved secret values no longer land on disk during automated platform setup. Success messages for `register`, `rotate-secret`, `build`, and `resolve` updated to reflect the new mental model.

### Fixed
- **`aifabrix run` for external / `integration/` apps:** When the app is not started as a local container, **Next actions** now suggest **`aifabrix upload <app>`** and **`aifabrix deploy <app>`** (with clearer guidance) instead of **`aifabrix build`**, which does not apply to external integrations.
...
### Technical
- **`aifabrix run` external branch:** `lib/app/run.js` exports **`validateAppForRun`** for unit tests; **`tests/lib/app/run-validate-app-for-run.test.js`** asserts upload/deploy next actions (not build).
- **Plan 139 follow-up (resolve / run env parity):** `lib/utils/env-copy.js` — `syncWrittenBuilderEnvToOutputPath`, `processEnvVariables` copies the same resolved `<appPath>/.env` to `build.envOutputPath` when `appName` is set and that file exists (fallback: `writeLocalEnvToOutputPath`); `mergeEnvMapIntoContent` optional **`appendMissingFromNewMap`**; `writeEnvOutputForReload` seeds from **`local`** template when output missing and merges **`.env.run`** without appending new keys. `lib/app/run-helpers.js` — `writeEnvOutputIfConfigured` restores **`writeEnvOutputForLocal`** for non-reload. `lib/core/secrets-env-write.js` — JSDoc for `resolveAndWriteEnvFile`. Tests: `tests/lib/utils/write-env-output-reload.test.js`, `tests/lib/core/secrets.test.js`, `tests/lib/app/app-run-helpers.test.js`.
...
- **Plan 140:** `lib/utils/installation-log.js`, `installation-log-record.js`, `installation-log-core.js`, `lib/cli/installation-log-command.js`, `setup-infra-up-platform-action.js`, `setup-infra-up-dataplane-action.js`; tests `tests/lib/utils/installation-log.test.js`, `tests/lib/utils/log-redaction.test.js`.

## [2.44.6] - 2026-05-06

### Added
- **`aifabrix datasource capability relate`:** Adds or replaces **`foreignKeys[]`** metadata (and optional **`metadataSchema`** property stub) with **`--relation-name`**, **`--to`**, **`--field`**. Plan 132 Phase 4.
- **`aifabrix datasource capability dimension`:** Adds or replaces one root **`dimensions.<key>`** binding (local field or FK traversal) with **`--dimension`**, **`--type local|fk`**, **`--field`** / **`--via`**. Includes semantic validation (FK/via checks), best-effort remote target validation, and optional online dimension-catalog enforcement when authenticated. Plan 135.
- **`aifabrix datasource capability create`:** **`--openapi-operation`** (match **`operationId`** in-file) and **`--template minimal-fetch`** (shipped template) as alternatives to **`--from`**. Plan 132 Phase 3.
...
### Changed
- **Health checks (`aifabrix run`, `up-miso`, `up-dataplane`, `up-platform`, guided setup):** When Traefik/public URL probing is enabled but the hostname does not resolve (DNS), or when checks succeed only via localhost while a public URL was listed first, the CLI prints a single **⚠** warning after **✔ Application is healthy** with the full URL and guidance to validate DNS, Traefik routing, and TLS. Guided **`waitForAppReady`** now respects **`traefik: true`** in user config the same way as **`aifabrix run`**.
- **`kv://` resolve / `.env` generation:** **`loadSecrets`** again merges the Builder **shared secrets HTTP API** when **`aifabrix-secrets`** is an **`http(s)`** URL (same order as elsewhere: user **`~/.aifabrix/secrets.local.yaml`** wins on overlaps; remote fills gaps). **`af run`** / registry token injection use this merged map so **`kv://`** matches keys visible under **`aifabrix secret list --shared`**. Missing-secret messages still label the configured shared API URL when **`aifabrix-secrets`** is remote.
- **`generateEnvContent` / `af run` (persist resolved keys locally):** After **`kv://`** resolution succeeds, each referenced key that is **missing or empty** in the primary user secrets file (`~/.aifabrix/secrets.local.yaml`) is **written** with the resolved value (same merge order as **`loadSecrets`**). Existing **non-empty** local values are **not** overwritten. Skipped when an explicit **`secretsPath`** is passed to **`generateEnvContent`**; tests may set **`skipMaterializeKvToLocal`**. Helps keep reinstall / regenerated `.env` aligned with databases when values first came only from shared API or merged YAML.
- **`ensureInfraSecrets` / `up-infra`:** Generated infra keys are written **only** to the primary **user** secrets file (`~/.aifabrix/secrets.local.yaml`), **not** to the Builder shared HTTP API—even when **`aifabrix-secrets`** is an **`http(s)`** URL. **`kv://`** resolution still merges shared remote keys via **`loadSecrets`**. To put a value on shared intentionally, use **`aifabrix secret set --shared <KEY> <value>`**.
- **Infra bootstrap UX (`up-infra`, `up-miso`, `up-dataplane`, `up-platform`, `down-infra`):** Default output is now a guided, product-oriented flow with a concise “Ready” footer (URLs and environment). Use **`--verbose`** on these commands to show full orchestration details (compose generation, health polling, deployment polling, container details). Guided footers resolve public URLs using existing Builder URL resolution (Traefik/frontDoorRouting/remote-server/tls) instead of assuming `localhost`.
...
- **External integration README generation:** External-system README now includes `wizard.yaml` and `rbac.*` in the Files list, uses copy/paste-ready `af ds cap …` examples with a real datasource key, and derives `aifabrix secret set …` hints from `integration/<app>/env.template` when present.
...
- **Tests (Plan 132 hardening):** Isolated real-fs capability tests in `jest.projects.js` to avoid `jest.mock('fs')` bleed (now includes copy/diff/edit/remove), and added negative-path coverage for capability create/relate (invalid inputs, not-found/ambiguous OpenAPI operationId, missing templates, overwrite guards).
...
- **Plan 132:** `lib/datasource/capability/*`, `lib/commands/datasource-capability.js`; datasource validate wiring; CLI matrix/docs updates for capability leaf commands; additional negative CLI/unit tests.
- **Infra guided UX:** `lib/cli/infra-guided.js`, `lib/utils/with-muted-logger.js`, and `lib/cli/setup-infra.js` updates for guided-mode output, compact device login output, and `--verbose` escape hatch.
...
### Changed
- **`aifabrix up-platform --force`:** Before cleaning builder app folders, clears all stored device and client tokens in local config, sets `environment` to `dev`, sets `controller` to the default URL for the current developer ID (same port rule as local miso-controller), then re-fetches keycloak, miso-controller, and dataplane from templates. Re-login required afterward.
...
- **build.reloadStart for run --reload (plan 72):** Optional **build.reloadStart** in `application.yaml` — when running `aifabrix run <app> --reload`, the container command can be overridden with this string (run from `/app`), e.g. TypeScript `pnpm run reloadStart`, Python `make reloadStart`. Compose generator passes `reloadStartCommand` into template data when both `devMountPath` and `build.reloadStart` are set; TypeScript and Python docker-compose templates emit `command: ["sh", "-c", "cd /app && <reloadStart>"]`. Schema, miso-controller and dataplane application templates, docs (application-yaml.md, running.md).
- **Secrets env docs, install command, and script mapping (plan 73):** **Docs:** env-template.md and application-development.md document where **build**, **run**, **shell**, and **install** get environment (NPM_TOKEN, PYPI_TOKEN via kv://); build does not receive secrets today; run/shell/install use resolved .env. **CLI:** New `aifabrix install <app> [--env dev|tst]` — same pattern as test (`getInstallCommand` from build.scripts.install or `pnpm install`/`make install`; dev = exec in running container, tst = ephemeral). Optional `aifabrix test-e2e <app>` and `aifabrix lint <app>` with same pattern. **Docs:** Table mapping test, install, test-e2e, lint to build.scripts keys and npm/make defaults (application-yaml, application-development); developer-isolation.md updated.
...
- **Docs (plan 67):** application-yaml, developer-isolation, running, building, configuration (env-config, env-template, secrets-and-config), commands (application-development, developer-isolation, infrastructure, utilities, reference, README), and top-level docs updated for single .env, `build.context` as content mapping, removal of `build.localPort`, remote dev config, and post-65/66 behaviour.

### Technical
- **Vector DB convention:** `lib/utils/compose-generator.js` — `isVectorDatabase` helper and `isVectorDatabaseName`; `templates/python/docker-compose.hbs`, `templates/typescript/docker-compose.hbs` — db-init `CREATE EXTENSION vector` for names ending with `vector`; `tests/lib/compose-generator.test.js` — vector DB and non-vector DB cases, helper unit test; `docs/running.md`, `docs/configuration/application-yaml.md` — convention and Azure note.
- **Remote dev:** `lib/api/dev.api.js`, `lib/commands/dev-init.js`, `lib/commands/dev-down.js`, `lib/commands/dev-cli-handlers.js`; `lib/utils/remote-dev-auth.js`, `lib/utils/remote-docker-env.js`, `lib/utils/remote-secrets-loader.js`, `lib/utils/mutagen.js`, `lib/utils/ssh-key-helper.js`, `lib/utils/dev-cert-helper.js`; config keys and GET /api/dev/settings usage; Mutagen session lifecycle and path formula (user-mutagen-folder + `/` + (build.remoteSyncPath if set, else `dev/` + appKey)).
...
- **Plan 69:** `lib/schema/application-schema.json` — `build.remoteSyncPath` (optional); `lib/utils/mutagen.js` — `getRemotePath(..., remoteSyncPath)`; `lib/app/run.js` — `isLocalhostEndpoint`/`isLocalhostHost`, localhost guard in `ensureReloadSync`, `remoteSyncPath` passed from app config; `tests/lib/utils/mutagen.test.js`, `tests/lib/app/run-reload-sync.test.js`; docs (application-yaml, running, builder-cli, application-development, building).
- **Plan 70:** `lib/api/credential.api.js`, `lib/api/types/credential.types.js` — `storeCredentialSecrets` (POST /api/v1/credential/secret); `lib/utils/credential-secrets-env.js` — KV_* → kv://, resolve kv:// in values, collect kv:// from payload, `pushCredentialSecrets`; `lib/commands/upload.js` — call push before runUploadValidatePublish; docs (external-integration, secrets-and-config, permissions); `tests/lib/api/credential.api.test.js`, credential-secrets-env and upload tests.
- **Plan 71:** `lib/datasource/field-reference-validator.js` — `validateFieldReferences(parsed)`; `lib/datasource/validate.js` — run field-reference check after schema validation; `tests/lib/datasource/field-reference-validator.test.js`, `tests/lib/datasource/datasource-validate.test.js`.
- **Plan 72:** `lib/schema/application-schema.json` — `build.reloadStart` (optional string); `lib/utils/compose-generator.js` — `reloadStartCommand` in template data when devMountPath and reloadStart set; `templates/typescript/docker-compose.hbs`, `templates/python/docker-compose.hbs` — conditional command block; `templates/applications/miso-controller/application.yaml`, `templates/applications/dataplane/application.yaml` — reloadStart example; docs (application-yaml.md, running.md); `tests/lib/compose-generator.test.js` — reloadStart cases.
- **Plan 73:** `lib/commands/app-install.js` — getInstallCommand, runAppInstall; `lib/cli/setup-app.js` — install (and optional test-e2e, lint); docs (env-template, application-development, application-yaml, developer-isolation); optional build.scripts schema; `tests/lib/commands/app-install.test.js`.
