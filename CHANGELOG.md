...
- **Secret resolution — in-memory only for platform bring-up (Plan 139):** `aifabrix register`, `aifabrix rotate-secret`, `aifabrix build`, and `aifabrix up-platform` / `up-miso` / `up-dataplane` no longer write `<appPath>/.env` or `build.envOutputPath`. `generateEnvFile` now accepts a `noWrite` option that resolves `kv://` references in memory (so missing-secret errors still surface) but skips both writes. The only commands that materialize a persistent `.env` are **`aifabrix resolve <app>`** (explicit user request) and **`aifabrix run <app>`** (ephemeral `.env.run` / `.env.run.admin`, removed after the container is healthy). ISO 27001 alignment: resolved secret values no longer land on disk during automated platform setup. If a tool or IDE needs an on-disk `.env`, run `aifabrix resolve <app>` once. Success messages for `register`, `rotate-secret`, `build`, and `resolve` updated to reflect the new mental model.
