...
- [ ] Read CLI Command Development and Quality Gates in [project-rules.mdc](.cursor/rules/project-rules.mdc).
- [ ] Read [docs-rules.mdc](.cursor/rules/docs-rules.mdc) for any `docs/` edits (if applicable).
- [ ] Read [layout.md](layout.md) for color semantics and block styling.
- [ ] Trace `ApiClient` unwrap for `POST /api/v1/pipeline/upload` vs raw `PublicationResult` before coding display.
- [ ] Confirm `GET /api/v1/external/systems/{key}` response shape for datasource list vs manifest-only count.
