...
`tests/setup.js` clears `AIFABRIX_HOME`, `AIFABRIX_WORK`, and `AIFABRIX_CONFIG` on each test unless `PRESERVE_AIFABRIX_TEST_ENV=true`. **Once per Jest process**, `tests/global-live-fabrix-backup.js` (globalSetup/globalTeardown on `jest.config.default.js` / `jest.config.isolated.js`) backs up and restores live operator dirs (`~/.aifabrix`, including symlink targets such as `/workspace/.aifabrix`) via `discoverLiveFabrixRuntimeConfigDirs` + `tests/helpers/aifabrix-runtime-backup.js`. `pnpm test:ci` (`tests/scripts/ci-simulate.sh`) also runs `tests/scripts/live-fabrix-backup-cli.js backup|restore` around `npm test` so secrets are restored even if Jest is killed by timeout. Optional tmp sandbox: `initAifabrixJestSandbox()` in `tests/helpers/aifabrix-runtime-sandbox.js` (see `paths-jest-sandbox-integration.test.js`). Secret and `admin-secrets.env` writes/deletes during Jest **refuse the live** tree even when `CI=true` / `CI_SIMULATION=true` (`assertWritableSecretsPathForTests`, mocked `fs` + `__AIFABRIX_NODE_FS_UNMOCKED__`, including `secret remove-all` and setup `removeUserLocalSecrets`); mock homes (e.g. `/home/test/.aifabrix`) and tmpdirs remain allowed. Opt out with `ALLOW_AIFABRIX_TEST_WRITE_REAL_CONFIG=1`. With `PRESERVE_AIFABRIX_TEST_ENV=true`, only the env-pointed runtime dir is backed up/restored in `setup.js` afterAll.
