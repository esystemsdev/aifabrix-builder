...
describe('App Module - Uncovered Code Paths', () => {
  ...
  beforeEach(() => {
    ...
    // Templates should already exist (created by setup.js in CI)
    // We don't create them here to avoid hitting the global guard
    // If templates don't exist, setup.js will handle creation in CI environments
    ...
    // Set up default mock AFTER clearing - will be overridden by mockImplementationOnce in specific tests
    appConfig.generateConfigFiles.mockImplementation(async(appPath, name, config, existingEnv) => {
      ...
      try {
        // Create parent directory if it doesn't exist - mkdirSync with recursive creates all parents
        if (!fsSync.existsSync(parentDir)) {
          try {
            fsSync.mkdirSync(parentDir, { recursive: true, mode: 0o755 });
          } catch (mkdirError) {
            // If directory was created by another process, that's fine
            if (mkdirError.code !== 'EEXIST' && !fsSync.existsSync(parentDir)) {
              throw new Error(`Failed to create parent directory ${parentDir}: ${mkdirError.message} (code: ${mkdirError.code})`);
            }
          }
        }
        ...
        if (!fsSync.existsSync(appPath)) {
          try {
            fsSync.mkdirSync(appPath, { recursive: true, mode: 0o755 });
          } catch (mkdirError) {
            // If directory was created by another process, that's fine
            if (mkdirError.code !== 'EEXIST' && !fsSync.existsSync(appPath)) {
              throw new Error(`Failed to create app directory ${appPath}: ${mkdirError.message} (code: ${mkdirError.code})`);
            }
          }
        }
  ...
  afterEach(async() => {
    // Don't restore mocks - they're module-level and should persist
    // Just clear call history for next test
    jest.clearAllMocks();
  ...
  describe('updateVariablesForAppFlag', () => {
    ...
    it('should handle error when updating application.yaml', async() => {
      ...
      // Ensure generateConfigFiles creates the directory and file
      appConfig.generateConfigFiles.mockImplementationOnce(async(appPath, name, config, existingEnv) => {
        const parentDir = path.dirname(appPath);
        try {
          // Create parent directory if it doesn't exist
