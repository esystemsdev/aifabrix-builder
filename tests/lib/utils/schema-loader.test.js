...
/**
 * @returns {{ existsSync: jest.Mock, readFileSync: jest.Mock, restore: Function }}
 */
function stubRealFsSync() {
  const existsSync = jest.spyOn(fsRealSync, 'existsSync');
  const readFileSync = jest.spyOn(fsRealSync, 'readFileSync');
  return {
    existsSync,
    readFileSync,
    restore() {
