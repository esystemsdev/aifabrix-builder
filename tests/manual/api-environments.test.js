...
describe('Manual API tests – environments.api (real Controller)', () => {
  ...
  it('GET /api/v1/environments/{envKey}/applications/{appKey} returns app when exists', async() => {
    ...
    const listRes = await listEnvironmentApplications(controllerUrl, environment, authConfig, {
      pageSize: 20
    });
    if (!listRes?.success) {
      return;
    }
    const items = applicationItemsFromListResponse(listRes);
    if (items.length === 0) {
      return;
    }
    ...
    for (const item of items) {
      const appKey = applicationKeyFromListItem(item);
      if (!appKey) {
        continue;
      }
      lastResponse = await getEnvironmentApplication(controllerUrl, environment, appKey, authConfig);
